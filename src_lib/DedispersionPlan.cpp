    ...
    config(config_)
{
    ...
    for (long ipri = 0; ipri < num_primary_trees; ipri++) {
        ...
        for (long et_level = num_early_triggers; et_level >= 0; et_level--) {
            ...
            // config. Fallback if the kernel is not compiled into this build:
            // time_downsampling. In that case a GpuDedisperser can't be constructed
            // anyway, so tokens can only come from a ReferenceDedisperser, whose
            // historical convention is Dcore = time_downsampling (= Dout).
            tree.Dcore = CoalescedDdKernel2::get_registry_dcore(dtype, tree, tree.pf.time_downsampling);
