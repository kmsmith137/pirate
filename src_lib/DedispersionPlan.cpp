    ...
    config(config_)
{
    ...
    for (long itree = 0; itree < ntrees; itree++) {
        ...
        // granularity of out_argmax tokens) is a compile-time property of the autogenerated
        // cdd2 kernel -- a registry value, not part of the registry key -- so we look it up
        // here. If the kernel is not compiled into this build, fall back to
        // time_downsampling: in that case a GpuDedisperser can't be constructed anyway, so
        // tokens can only come from a ReferenceDedisperser, whose historical convention is
        // Dcore = time_downsampling (= Dout).

        CoalescedDdKernel2::RegistryKey cdd2_key = CoalescedDdKernel2::_make_registry_key(kparams, pf_params);
        pf_params.Dcore = CoalescedDdKernel2::registry().has_key(cdd2_key)
            ? CoalescedDdKernel2::get_registry_dcore(kparams, pf_params)
...
void DedispersionPlan::to_yaml(YAML::Emitter &emitter, bool verbose, bool zones) const
{
    ...
    for (long tree_index = 0; tree_index < ntrees; tree_index++) {
        ...
        emitter << YAML::Key << "Dcore" << YAML::Value << stage2_pf_params.at(tree_index).Dcore;
        if (verbose)
