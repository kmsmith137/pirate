...
static void _verify_tree_yaml(const YamlFile &yt, long itree, const DedispersionTree &ref)
{
    stringstream ctx_ss;
                                                        ...
                                                        const YamlFile &plan_yaml)
{
    ...
    for (long itree = 0; itree < plan->ntrees; itree++)
        _verify_tree_yaml(ytrees[itree], itree, plan->trees.at(itree));
...
void DedispersionPlan::decode_argmax(
    uint argmax_token, long itree, long Dcore, long idm_coarse, long itime_coarse,
