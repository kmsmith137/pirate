...
void SdPlan::plan_pass()
{
    ...
    // y_true and hence get_distance(). This is the analogue of PfAvarApproximation's m_cnt >= 1
    // assert.
...
void SdPlan::emit(long ifreq, const SparseTile &tile, long klev, uint64_t sdbits)
{
    ...
    // quadratic (this is what PfVariance::add_tile() does); omitting it is silently wrong wherever
    // an edge tile deferred its 1/sqrt(2). The second is the level-r normalization above. They
