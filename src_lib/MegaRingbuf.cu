...
//
//         long consumer_num_views[num_consumers];
//
...
//     views by an index:
//
//       0 <= iview < consumer_num_views[consumer_id]
//
...
struct MegaRingbuf {
    ...
    static constexpr int MCP = max_consumers_per_producer;  // shorthand

    // Parameters specified at construction.
    struct Params {
        long total_beams = 0
        long max_chunk_lag = 0;
        long max_gpu_chunk_lag = 0;
        bool hugepages = 0;

        // Number of segments (or "views", see above) for each producer/consumer.
        std::vector<long> producer_num_views;  // Length (num_producers)
        std::vector<long> consumer_num_views;  // Length (num_consumers)
    };
    ...
    void add_segment(
        ...
        long producer_iview, 
        int num_consumers, 
        int consumer_ids[MCP], 
        long consumer_iviews[MCP],
        long chunk_lags[MCP]
    );
    ...
    // Allocates memory on both CPU and GPU.
    void allocate();

    // The "giant" buffers contain all zones.
    ...
    {
        bool on_gpu = false;
        long num_frames = 0;             // specified at construction
        long segments_per_frame = 0;     // starts at 0, and increases as add_segment() is called.
        long giant_segment_offset = 0;   // segment offset relative to {cpu,gpu}_giant_buffer.base
    };
