    ...
    params(params_)
{
    ...
    int A = pow2(params.input_toplevel_rank - params.output_dd_rank - 1);
    ...
    for (int ipri = 0; ipri < params.num_primary_trees; ipri++) {
        int r = params.input_toplevel_rank - (ipri ? 1 : 0);
...
void DownsamplingKernelImpl<T>::launch(DedispersionBuffer &buf, long ichunk, long ibatch, cudaStream_t stream)
{
    ...
    // 'input_toplevel_rank' is communicated via A = A_B * A_W = pow2(input_toplevel_rank - output_dd_rank - 1).
...
static DedispersionBuffer make_buffer(const LaggedDownsamplingKernelParams &lds_params, int aflags)
{
    ...
    for (long ipri = 0; ipri < lds_params.num_primary_trees; ipri++) {
        long rk = lds_params.input_toplevel_rank - (ipri ? 1 : 0);
...
void GpuLaggedDownsamplingKernel::test_random()
{
    ...
    cout << "    ti.nchunks = " << ti.nchunks << ";" << endl;
    run_test(ti);
}
...
// -------------------------------------------------------------------------------------------------
//
// GpuLaggedDownsamplingKernel::time() implementation


namespace {

// Helper function for time(), also defined in DedispersionBuffer.cu
DedispersionBuffer make_lds_timing_buffer(const LaggedDownsamplingKernelParams &lds_params, int aflags)
{
    DedispersionBufferParams dd_params;
    dd_params.dtype = lds_params.dtype;
    dd_params.beams_per_batch = lds_params.beams_per_batch;
...
void time_gpu_lagged_downsampling_kernel(const LaggedDownsamplingKernelParams &params)
{
    ...
    for (long s = 0; s < nstreams; s++)
        bufs.push_back(make_lds_timing_buffer(params, af_zero | af_gpu));
    ...
    kernel_name << "gpu_lagged_downsample("
                << "dtype=" << params.dtype
                << ", input_toplevel_rank=" << params.input_toplevel_rank
