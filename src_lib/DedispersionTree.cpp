                                   ...
                                   bool Dcore_from_cdd2_registry)
{
    ...
    // time_downsampling is what becomes the cdd2 registry key's Dout, so this line and
    // makefile_helper.autogenerated_cdd2_kernels()'s emit loop state the same invariant.
    this->dm_downsampling = pow2(dd_rank1);
    this->time_downsampling = pow2(dd_rank1);

    // All four downsampling factors are powers of two: the wt factors are checked by
    // config.validate(), and the two output factors are pow2() by the assignments just
    // above. Assert all four here -- where the resolved values are first established and
    // much downstream code assumes the property.
    xassert(is_power_of_two(this->dm_downsampling));
    xassert(is_power_of_two(this->time_downsampling));
    xassert(is_power_of_two(this->pf.wt_dm_downsampling));
    xassert(is_power_of_two(this->pf.wt_time_downsampling));

    // The two wt_* lower bounds are also checked by config.validate() (with a message that
    // names the offending primary tree), so these asserts are a backstop on the per-tree
    // values rather than the user-facing check. The time bound is what makes the xdiv()
    // below exact: time_downsampling <= wt_time_downsampling <= nt_ds, all powers of two.
    xassert_le(this->dm_downsampling, this->pf.wt_dm_downsampling);
    xassert_le(this->pf.wt_dm_downsampling, pow2(tot_rank));
    xassert_le(this->time_downsampling, this->pf.wt_time_downsampling);
    ...
    this->nt_out = xdiv(this->nt_ds, this->time_downsampling);
...
void DedispersionTree::check_consistency(const DedispersionConfig &config) const
{
    ...
    // The peak-finding factors. Note {dm,time}_downsampling are not config fields at all --
    // the tree constructor pins both to pow2(dd_rank1) -- so this is where a change to that
    // rule surfaces.
    cmp("pf.max_width", this->pf.max_width, ref.pf.max_width);
    cmp("dm_downsampling", this->dm_downsampling, ref.dm_downsampling);
    cmp("time_downsampling", this->time_downsampling, ref.time_downsampling);
...
DedispersionTree DedispersionTree::from_yaml(const YamlFile &yt, const DedispersionConfig &cfg)
{
    ...
    // '{dm,time}_downsampling' are tree members, not config fields: the constructor pins
    // both to pow2(dd_rank1). Transcribed verbatim here, like every other member -- the
    // pinning rule is deliberately not reimplemented.
    tree.dm_downsampling = yt.get_scalar<long>("dm_downsampling");
    tree.time_downsampling = yt.get_scalar<long>("time_downsampling");

    // 'pf' is seeded from the config's PrimaryTree (for num_early_triggers and the wt_*
    // factors), then max_width is overwritten from the yaml.
