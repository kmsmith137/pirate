                        ...
                        float eta, float eps, TimeStencils<NDEG,W> tb)
{
    ...
    for (int j = 0; j < nphi_z; j++) {
        ...
        for (int bd = 0; bd <= NPHI; bd++) {
            ...
            // Fold the window by parity: even and odd combinations of the s>0 and s<0
            // halves, plus the centre.
            const float ctr = zz[W];
            float ev[W], od[W];
            #pragma unroll
            for (int k = 1; k <= W; k++) {
                ev[k-1] = zz[W+k] + zz[W-k];
            ...
            for (int p = 0; p < NPAIR_T; p++) {
                float a = tb.gs[p][0] * ctr;
                const bool even = (tb.gpar[p] > 0.0f);
                #pragma unroll
                for (int k = 1; k <= W; k++)
                    a += tb.gs[p][k] * (even ? ev[k-1] : od[k-1]);
                Mc[p] = a;
            }
        ...
        // --- the right-hand side.
        {
            ...
            for (int q = 0; q <= NDEG; q++) {
                ...
                #pragma unroll
                for (int k = 1; k <= W; k++)
                    a += tb.us[q][k] * (even ? ev[k-1] : od[k-1]);
                u[(j*NQ + q)*S + tid] = a;
            }
        }
    }
    ...
    eta(eta_), eps(eps_)
{
    ...
    if ((n_phi == 2) && (n == 2) && (W == 4)) {
        TimeStencils<2,4> *tb = new TimeStencils<2,4>;
        fill_stencils<2,4>(*tb, P);
        tb_blob = tb;
    }
    else
...
{
    delete reinterpret_cast<TimeStencils<2,4> *>(tb_blob);
    tb_blob = nullptr;
}
