...
void test_gpu_tree_gridding_kernel()
{
    ...
    params.ntime = v[2] * xdiv(1024, params.dtype.nbits);
    params.nchan = rand_int(1, 2000/(v[1]*v[2]));
    ...
    Array<float> hsrc({B,F,T}, {hs_in,T,1}, af_uhost | af_random);
    Array<float> hdst({B,N,T}, {hs_out,T,1}, af_uhost | af_zero);
    
    Array<void> gsrc(params.dtype, {B,F,T}, {gs_in,T,1}, af_gpu | af_zero);
    Array<void> gdst(params.dtype, {B,N,T}, {gs_out,T,1}, af_gpu | af_zero);
