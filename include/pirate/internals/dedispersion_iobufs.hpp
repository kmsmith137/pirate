...
// Defined in DedispersionKernel.hpp
class GpuDedispersionKernel;


// This file defines helper classes:
//
//   dedispersion_simple_inbuf<T,Lagged>
//   dedispersion_ring_inbuf<T>
//   dedispersion_simple_outbuf<T,Lagged>
//   dedispersion_ring_inbuf<T>
//
// that are used to "glue" the host-side code in GpuDedsipersion.cu
// to the cuda kernels in template_instantiations/dedisp*.cu.
...
struct dedispersion_ring_outbuf
{
	...
	__device__ __forceinline__ device_state(const device_args &args, int active_rank, long rb_pos)
	{
	    ...
	    this->rb_base = args.rb_base;
	    this->rb_loc = args.rb_loc + (ambient_ix << active_rank) + (threadIdx.x >> 5);
	    this->rb_loc_seg_stride = (nambient << active_rank);
	    this->rb_pos = rb_pos + beam_ix;
	}
