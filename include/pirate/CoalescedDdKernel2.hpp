...
//   - pf_rank <= dd_rank1
//   - pf_params.dm_downsampling == 2^dd_rank1   (one output DM per warp of the second stage)
//   - pf_params.time_downsampling == the factor the kernel was compiled with
//     (RegistryValue::time_downsampling; 2^dd_rank1 for every kernel in the build, see
//     makefile_helper.autogenerated_cdd2_kernels())
//   - pf_params.nt_out * pf_params.time_downsampling == dd_params.ntime
//
...
// where K = (dd_rank1 - pf_rank) = log2(pf_params.dm_downsampling) - pf_rank. See
// notes/dedispersion.tex for discussion. The token format is:
//
...
{
    // Note: pf_params.{dm,time}_downsampling must match the factors the kernel was compiled
    // with (RegistryValue::{dm,time}_downsampling); the constructor checks this.
    ...
    PeakFindingKernelParams pf_params;   // beams_per_batch, total_beams, {dm,time}_downsampling, {ndm,nt}_{out,wt}
