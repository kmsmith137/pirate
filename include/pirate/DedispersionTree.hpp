...
struct DedispersionTree
{
    ...
    // assigned the placeholder value time_downsampling, which is ReferenceDedisperser's
    ...
    // copy of config.primary_trees[primary_tree_index]: the constructor resolves nothing
    // into it (the resolved factors are the tree members just below).
    ...
    // Time downsampling factor of the coarse-grained array, relative to this tree. Equal to
    // dm_downsampling: the output array's two axes are coarse-grained by the same factor.
    //
    // ALSO NOT A CONFIG FIELD, and that is load-bearing rather than cosmetic. nt_out =
    // nt_ds / time_downsampling, so this value IS the cdd2 registry key's 'Dout' (see
    // CoalescedDdKernel2::_make_key). Pinning it to pow2(dd_rank1) is therefore a statement
    // about which GPU kernels can exist: makefile_helper.autogenerated_cdd2_kernels()
    // compiles cdd2 kernels at that Dout only, and a kernel at any other Dout would be
    // unreachable from any config. The two must move together.
