...
//
// The GpuDedispersionKernel does not implement the 'sb_out' argument from the
// ReferenceDedispersionKernel. Two other kernels do: 'struct CoalescedDdKernel2',
// which fuses peak-finding into the second dedispersion stage so that the subband
// array is never materialized (this is what the production dedisperser uses), and
// 'struct GpuSbDedispersionKernel' at the bottom of this file, which writes the
// subband array out.
//
...
// multiplets -- the last ones, (M - 2^pf_rank) <= m < M -- together with the 'Dpf' coarse
// DMs are exactly the 2^(amb_rank + dd_rank) DM outputs which a GpuDedispersionKernel
// would have written.
//
...
// Defined in DedispersionKernel.cu
