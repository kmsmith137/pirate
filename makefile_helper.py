...
def _make_random_pf_weight_reader_kernels():
    """
    This function doesn't get run in the build process. I ran it once, to 
    to get the random kernels in generate_pf_weight_reader_test_kernels(),
    ...
    for _ in range(15):
        ...
        nbits = 32 // randi(1,3)
        rank = randi(2,5)
        subband_counts = [ randi(2**rank) ] + [ randi(2**(rank-l+1)-1) for l in range(1,rank) ] + [ 1 ]
        Tinner_log = randi(6)
...
def generate_pf_weight_reader_test_kernels():
    """Generates filenames."""
    ...
    # See _make_random_pf_weight_reader_kernels() above.
    ...
    for (dtype, subband_counts, Dcore, P, Tinner) in kernels:
        ...
        k = cuda_generator.PfWeightReader(fs, dtype, Dcore, P, Tinner)
        yield f'src_lib/autogenerated_kernels/{k.test_kernel_basename}'
        
    
def generate_pf_output_test_kernels():
    """Generates filenames."""

    # In this case, we can enumerate all valid parameter choices.
    for (dtype,Lmin) in [ ('float',0), ('__half',1) ]:
        for L in range(Lmin, 6):
            k = cuda_generator.PfOutput2(dtype, Dout=2**L)
            yield f'src_lib/autogenerated_kernels/{k.test_kernel_basename}'
            

####################################################################################################

_make_random_pf_weight_reader_kernels()
...
for k in generate_kernel_filenames():
    print(f' \\\n    {k}', end='', file=s)
for filename in generate_pf_weight_reader_test_kernels():
    print(f' \\\n    {filename}', end='', file=s)
for filename in generate_pf_output_test_kernels():
    print(f' \\\n    {filename}', end='', file=s)
print(file=s)
