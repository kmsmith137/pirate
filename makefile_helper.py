...
def autogenerated_cdd2_kernels():
    ...
    #   (dtype, dd_rank, Wmax, Dcore, Dout, Tinner, subband_counts, et_delta_ranks)
    ...
    # Note: we use dd_rank <= 5 here, since DedispersionConfig::make_random() uses default max_rank=10.
    kernels = [ 
        ('fp32', 4, 16, 8, 16, 1,   (2,2,1),   [0,1]),
        ('fp32', 5, 16, 8, 16, 1,   (2,2,1), [0,1,2]),
        ('fp16', 4,  8, 8,  8, 2, (2,3,1,1),   [0,1]),
        ('fp16', 5,  8, 8,  8, 2, (2,3,1,1), [0,1,2]),
        ('fp32', 5, 16, None, None, 2, (2,2,1), [0,1,2])  # needed for configs/dedispersion/toy.yml
    ]
    ...
    if production_kernels:
        ...
        # triples = (dd_rank, subband_counts, et_delta_ranks)
        triples = [ 
        ...
        for dd_rank, subband_counts, et_delta_ranks in triples:
            # Values common to all chime/chord kernels.
            # FIXME I'd like to use Wmax=32, but that currently fails!
            dtype, Wmax, Dcore, Dout, Tinner = ('fp16', 16, None, None, 1)
            kernels += [ (dtype, dd_rank, Wmax, Dcore, Dout, Tinner, subband_counts, et_delta_ranks) ]
    ...
    for dtype, dd_rank0, Wmax, Dcore0, Dout0, Tinner, subband_counts, et_delta_ranks in kernels:
        for et_delta_rank in et_delta_ranks:
            dd_rank = dd_rank0 - et_delta_rank
            pf_rank = (dd_rank+1) // 2
            Dout = Dout0 if (Dout0 is not None) else 2**pf_rank
            Dcore = Dcore0 if (Dcore0 is not None) else min(Dout,8)

            sbc = cuda_generator.FrequencySubbands.restrict_subband_counts(subband_counts, et_delta_rank, pf_rank)
