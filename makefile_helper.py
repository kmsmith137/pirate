...
def autogenerated_cdd2_kernels():
    ...
    for dtype, dd_rank0, subband_counts, Wmax, Dcore, Dout, Tinner in kernels:
        # For every cdd2 kernel in the 'kernels' list, we generate a few more kernels
        # which are related by downsampling or 
        ...
        for ds_rank in [dd_rank0-1, dd_rank0]:   # downsampled dd rank
            for et_rank in [ds_rank-2, ds_rank-1, ds_rank]:
                ...
                fs = cuda_generator.FrequencySubbands(subband_counts = sc)
                cdd2 = cuda_generator.CoalescedDdKernel2(dtype, et_rank, fs, Wmax, Dcore, Dout, Tinner)
                yield cdd2
