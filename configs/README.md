...
The top-level file `asdf_header.yml` is auto-generated by the Makefile
(`python -m pirate_frb show_file_format configs/xengine/xengine_metadata_v2.yml`)
and is checked into git as a documentation snapshot of the verbose ASDF
YAML header emitted by `AssembledFrame::write_asdf()`. Do not edit it
by hand; regenerate by running `make`.
