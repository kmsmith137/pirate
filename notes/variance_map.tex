...
Note that \verb|max_r| is an {\em inflation factor}, not an accuracy measure. It is a
per-element {\em relative} quantity with no floor, so on a matrix with wide dynamic range it
is set by the smallest element compared rather than by how well the matrix is approximated:
a map accurate to the float64 noise floor in every absolute sense can report
$\mbox{\tt max\_r}-1 \sim 10^{-9}$ simply because its worst element lies a couple of decades
below the matrix maximum. The same method therefore also reports
\begin{equation}
{\tt max\_diff} \equiv \frac{\max_{\alpha F} \big| A^{\rm true}_{\alpha F}
                                              - A^{\rm approx}_{\alpha F} \big|}
                            {\max_{\alpha F} \big| A^{\rm true}_{\alpha F} \big|}
\end{equation}
which is the sup-norm accuracy of $A^{\rm approx}$ relative to $A^{\rm true}$'s own scale, and
is zero for an exact map. The two answer different questions and generally attain their
maxima at different elements: read \verb|max_r| for whether an approximation can be
{\em used}, and \verb|max_diff| for how {\em good} it is. In particular an approximation which
is negative somewhere $A^{\rm true}$ is positive has $\mbox{\tt max\_r} = \infty$ (no
rescaling repairs it) while its \verb|max_diff| can be perfectly small.
