...
\item One detail: when we either coarse-grain a map $A_{\alpha F} \rightarrow \bar A_{\beta F}$,
 ...
 $A^{\rm approx}_{\alpha F} \le A^{\rm true}_{\alpha F}$ is satisfied.
...
have been checked numerically against {\tt PfAvarExact} (float64) over random configs: the row
identities (\ref{eq:restriction_et}) and (\ref{eq:restriction_ds}) hold to $0$ and one ulp
respectively. Proposition 1 has additionally been checked with a {\tt Detrender2d} in front of
...
{\tt TmpVmapExact}. Under a bit-reversed DM index, aligned dyadic blocks become strided
...
\section{Analytic variances from sparse tiles}
...
This appendix describes how those variances are computed analytically, without simulation:
the compressed representation of a single input channel's impulse response through tree
dedispersion (\verb|SparseTile|, \verb|SparseTileTriple|), and the autocorrelation table that
turns such a response into a peak-finding output variance (\verb|PfVarianceConvolver|).
Both live in \verb|pirate_frb/varmap/|, with a C++ port in \verb|src_lib/varmap.cpp|.

{\bf Status of the code named below.} This appendix was originally written around a pair of
classes, \verb|PfAvarExact| (which evaluated $y = Av$ for one $v$, per multiplet) and
\verb|TmpVmapExact| (which compressed $A$ itself into a canonical low-rank form), together
with their approximate counterparts in \S\ref{sec:variance_svd}. All four were a first pass at
representing the variance map of \S\ref{sec:variance_map}, written before we settled on the
\verb|VarianceMap| representation, and all four have since been DELETED along with the
\verb|pirate_frb.slow_avar| subpackage that held them. What survives, and what the rest of this
appendix should be read as describing, is:
\begin{itemize}
\item \S\ref{sec:avar_statement}--\S\ref{sec:avar_pfvariance}: the math of the sparse-tile
representation and of the variance evaluation. This is LIVE. It is what
\verb|varmap/detrender_free.py| and its C++ port are built on, and it is what the real-time
server runs, via \verb|compute_detrender_free_varcoarse()|.
\item \S\ref{sec:avar_validation}: how the result is checked. Rewritten below to describe the
checks that now exist.
\item \S\ref{ssec:svd_exact_map}--\S\ref{ssec:svd_exact_cost} and \S\ref{sec:variance_svd}:
the two compression schemes, and the two drivers that fed them. The code is gone; the sections
are kept for the record, each behind a status note.
\end{itemize}
The current route to $A$ is \S\ref{ssec:varmap_proposed}'s \verb|VarianceMap| built by
\verb|varmap/detrender_free.py|, which groups channels by SUBBAND rather than by multiplet --
measured, $2.9$ to $5.7$ times fewer convolutions -- and, unlike \verb|PfAvarExact|, is
practical at CHIME/CHORD scale.
...
Everything below is run by \verb|pirate_frb test --varmap|. (There used to be a separate
\verb|--avar| flag for the pieces in this appendix; it was folded into \verb|--varmap| when
they moved into \verb|pirate_frb/varmap/|.)

The pieces above are unit-tested against dense reference implementations: the
...
\verb|PfVarianceConvolver| is compared both to a brute-force evaluation of $\|h_p * x\|^2$,
and to the kernels which \verb|ReferencePeakFindingKernel| actually uses. The C++ port of each
is compared to the python.

End-to-end, the analytic variances are checked against the BRUTE-FORCE SWEEP of
\S\ref{sec:variance_map_bruteforce}, which pushes a one-hot through the real dedisperser once
per input channel and reads $A$ off the output column norms. That comparison
(\verb|test_multimap_vs_sweep| in \verb|varmap/tests.py|) is elementwise over the full matrix,
for every primary tree, on random configs; the two sides share no code, and agreement is at
the float32 floor of the dedispersion chain, measured $5 \times 10^{-7}$ to
$2 \times 10^{-6}$. It is the only numerical oracle for the analytic map, and it covers
\verb|compute_detrender_free_varfine()| in the same loop.

Separately, a stored \verb|VarianceMap| can be checked by Monte Carlo
(\verb|pirate_frb varmap mc|): Gaussian noise with per-channel variance $V_F$ is fed through
the dedisperser, a \verb|PfSquare| is run over each tree's subband array (\verb|out_sb|), and
the resulting empirical per-chunk variances are compared to the map's prediction, restricted
to output channels which have reached steady state. This supersedes the older
\verb|check_avar_mc|, and unlike it, reports the worst UNDERestimate and the worst
OVERestimate separately -- an admissible map may do the second but never the first.

\clearpage

\subsection*{[The code below has been deleted]}

{\bf Status.} \verb|TmpVmapExact|, and the \verb|PfAvarExact| driver it was built on, were
deleted with the \verb|pirate_frb.slow_avar| subpackage. Their role -- compressing $A$ into a
low-rank form -- is filled by \verb|varmap/detrender_free.py| plus the covering LP of
\S\ref{ssec:varmap_proposed}, which does it at CHIME/CHORD scale, where {\tt PfAvarExact}
could not be built at all. The rest of this section is kept for the record: the per-group SVD
and canonical form it describes are still the shape of what the current code does, and
\S\ref{ssec:svd_exact_map} carries a definition that is referenced from earlier appendices.

The rest of this appendix describes \verb|TmpVmapExact|, which compresses the variance map
...
rank, so {\tt TmpVmapExact} inherits that and cannot be run at CHIME/CHORD scale.
...
numerical rank. The three factors are the {\tt TmpVmapExact} block members {\tt sv\_out},
...
floor while remaining far below any genuine component. (Both {\tt TmpVmapExact} and
{\tt TmpVmapApproximation} take an {\tt epsilon} constructor argument which overrides this
default.)
...
building {\tt TmpVmapExact} from it takes $11$ sec.
...
\section{{\tt PfAvarApproximation}, {\tt TmpVmapApproximation} [deleted]}
\label{sec:variance_svd}

{\bf Status: the code described here has been deleted.} {\tt PfAvarApproximation} and
{\tt TmpVmapApproximation} went with the \verb|pirate_frb.slow_avar| subpackage, and the C++
{\tt PfAvarApproximation} in \verb|src_lib/varmap.cpp| went with them. The real-time pipeline
had already stopped using them: {\tt GpuDedisperser::\_fill\_analytic\_weights()} now calls
\verb|compute_detrender_free_varcoarse()| instead, which is exact (no per-subband averaging)
and is the C++ port of \verb|varmap/detrender_free.py|. That swap cost a factor $3$ in
construction time at {\tt chord\_sb2\_et.yml} ($0.24$ s $\to$ $0.74$ s) and removed the
approximation entirely, so the inconsistency recorded in the next paragraph is gone with it.
The section is kept for the record.

This appendix documents a pair of classes ({\tt PfAvarApproximation}, {\tt TmpVmapApproximation})
which implemented a coarse-grained approximation to {\tt PfAvarExact}. They were a first pass at
representing the variance map, predating the {\tt VarianceMap} representation, and were
deliberately outside it.
These classes were written before the main body of these notes, and are inconsistent with it.
In particular, the distance $D(A^{\rm true}, A^{\rm approx})$ is infinite, where $D$ is defined
by Eq.\ (\ref{eq:distance_function}).
...
input vector $v$, and {\tt TmpVmapApproximation} compresses the map $A$ itself into a
...
(the deleted \verb|pirate_frb check_avar_approximation|, which reported the distribution of
...
{\tt PfAvarApproximation} builds in 13 sec and {\tt TmpVmapApproximation} in a further
...
{\tt PfAvarApproximation} was not built with ({\tt TmpVmapApproximation.check()}).
...
{\tt TmpVmapApproximation} must be built from the python class; at 13 sec this is not
