...
delete the old code. See \verb|plans/varmap_loose_ends.md| and
\verb|plans/varmap_new_vs_old.md| for current status.
...
\section{{\tt PfAvarExact} and {\tt TmpVmapExact}}
\label{sec:analytic_variance}

{\bf Status.} The four classes described in this appendix and in \S\ref{sec:variance_svd} --
{\tt PfAvarExact}, {\tt TmpVmapExact}, {\tt PfAvarApproximation}, {\tt TmpVmapApproximation} --
were a first pass at representing the variance map of \S\ref{sec:variance_map}, written before we
settled on the {\tt VarianceMap} representation. They are left unchanged for now, and are
deliberately outside it; the {\tt TmpVmap} prefix is the reminder. We may revisit them later.
...
\item \verb|TmpVmapExact| (\S\ref{ssec:svd_exact_map}--\S\ref{ssec:svd_exact_cost}) takes a
...
\verb|ReferenceDedisperser|, a \verb|ReferencePfSquare| is run over each tree's subband array
(\verb|out_sb|), and the resulting empirical per-chunk variances are compared to the analytic
prediction, restricted to output channels which have reached steady state.
...
\section{{\tt PfAvarApproximation}, {\tt TmpVmapApproximation} [deprecated]}
\label{sec:variance_svd}

This appendix documents a pair of classes ({\tt PfAvarApproximation}, {\tt TmpVmapApproximation})
which implement a coarse-grained approximation to {\tt PfAvarExact}. The status note at the top of
\S\ref{sec:analytic_variance} applies to them too: they were a first pass at representing the
variance map, predating the {\tt VarianceMap} representation, and are deliberately outside it for
now.
...
I'll resolve these inconsistencies later -- for now, you should consider the classes
{\tt PfAvarApproximation}, {\tt TmpVmapApproximation} to be a deprecated, temporary
kludge which is currently used in the real-time pipeline, but will be revised in the future.
