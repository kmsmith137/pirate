...
variance map $A^{(\gamma,e)}$, a $(D_{\gamma,e} M_{\gamma,e} P)$-by-$N_{\rm freq}$ matrix. Say
that $A'$ is a {\bf row-restriction} of $A$ if there is an injection from the rows of $A'$ to
the rows of $A$ under which corresponding rows are equal as vectors in
...
\subsection{Two consequences of (F1) and (F2)}
...
$0 \le j < 2^l$, every $0 \le d < D_\gamma$, every $p$ and every $F$,}
\begin{equation}
...
$\gamma$ only through $\sigma_\gamma \in \{0,1\}$, so {\em all} trees of a config with
$\gamma > 0$ and the same $e$ have {\em identical} variance maps, not merely nested ones --
the downsampling factor $2^\gamma$ itself never appears. Second, both propositions have been
checked numerically against {\tt PfAvarExact} (float64) over random configs: the row
