...
\newcommand{\ipnotes}[1]{{\em inpainting.pdf}, ``#1''}
...
\begin{enumerate}
\item Apply a {\tt GpuDetrenderLps2d} to the $(N_{\rm freq}, N_{\rm time})$ array
  in-place with {\bf no mask}. The parameters of the {\tt GpuDetrenderLps2d} are
...
\item There is a ``brute-force'' algorithm to compute the variance map $A$ in dense form 
 ...
 with time translations. In our case of interest, where $L$ is defined by a {\tt DetrenderLps2dParams}
...
\item In the ``detrender-free'' special case, there is a theorem that $A$ does not have
 ...
 Unfortunately, this algorithm does not generalize to the case where a {\tt GpuDetrenderLps2d}
 is specified.
...
\item In the general case (i.e.\ including a {\tt GpuDetrenderLps2d}), we're currently in the
...
and a {\tt DetrenderLps2dParams}, and written it to disk.
...
\item There is no subscale config in \verb|configs/dedispersion/| yet, so part of the task is to
 create a variety of them, exploring the {\tt DedispersionConfig} and {\tt DetrenderLps2dParams}
...
\item To start out, let's also disable frequency subbands, so that the
 main parameters to explore are the frequency zones, and the {\tt GpuDetrenderLps2d}.
...
Note the symbol clash with the {\tt GpuDetrenderLps2d} time half-width $W$ of \dtnotes{2-d detrending};
...
The exception is instructive. With {\em no} {\tt GpuDetrenderLps2d}, $A$ really is close to low rank
...
rank. Frequency subbands alone do not break this. It is the {\tt GpuDetrenderLps2d} that destroys the
...
The {\tt GpuDetrenderLps2d} is the one used throughout this study: a single spline zone with
...
\item The {\tt GpuDetrenderLps2d} time half-width is a large and cheap lever on all of this: raising it
...
\item {\bf The {\tt GpuDetrenderLps2d} knot vector as a design variable.}
...
\item In the ``no-detrender'' case (i.e.\ if the linear operator $L$ does not include a
 {\tt GpuDetrenderLps2d}), $A^{(\gamma,0)}$ is a row-restriction of $A^{(0,0)}$, for every $\gamma$.
\end{enumerate}
...
exactly as defined in \S\ref{ssec:variance_map_setup}, {\tt GpuDetrenderLps2d} included: the two
...
Now, {\tt max\_width} is a per-{\em primary}-tree config field, which the {\tt DedispersionPlan}
constructor copies verbatim onto each tree, so every tree of one family reads the same value; and
...
(\ref{eq:restriction_ds}) hold to $0$ and one ulp respectively. Proposition 1 has additionally been checked with a {\tt GpuDetrenderLps2d} in front of
...
operator of \S\ref{ssec:variance_map_setup}, which includes a {\tt GpuDetrenderLps2d}.
{\bf The result below is proved in that case only, and the restriction is not a
technicality:} with a {\tt GpuDetrenderLps2d} in front of the dedisperser the property is not
...
h_{\lambda,2} = \xi_3 B_{3w} + (1-\xi_3) B_w, \hspace{1cm}
h_{\lambda,3} = \xi_4 B_{4w} + (1-\xi_4) B_{2w},
\end{equation}
where in each case the boxcars are centered on the center of $h_{\lambda,q}$
(e.g.\ $[\xi_3, 1, \xi_3] = \xi_3 [1,1,1] + (1-\xi_3) [0,1,0]$), and the tapers satisfy
$0 \le \xi_3, \xi_4 \le 1$ (\ddnotes{Peak-finding kernels}), which is what makes these
combinations non-negative. Hence $A_p$ is a
...
{\tt GpuDetrenderLps2d} in front of the dedisperser: the effective per-channel response is no longer
...
{\tt GpuDetrenderLps2d}, which the machinery of \S\ref{sec:analytic_variance} cannot currently handle at
...
Besides the parameters of \S\ref{ssec:variance_map_setup}, we will need the {\tt GpuDetrenderLps2d} time
...
Every stage of $L$ is time-translation invariant. The {\tt GpuDetrenderLps2d} run with no mask is a fixed
...
{\em instantaneous} in time -- no detrender at all, or a {\tt GpuDetrenderLps2d} with $W=0$, which fits
...
\item {\bf Detrender padding.} The {\tt GpuDetrenderLps2d} is handed a length-$(T+2W)$ buffer and
...
range of a few tree-freq channels. A {\tt GpuDetrenderLps2d} destroys this: the detrended response to a
...
variance map $A$ of Eq.\ (\ref{eq:variance_map_def}) analytically -- no dedisperser is run and
no GPU is used -- and returns it directly in low-rank form $A = QW^T$. (It does build a
{\tt DedispersionPlan}, but a {\tt Params::minimal()} one, which is pure arithmetic: no
ring buffer, no kernel params, no CUDA device. That is the only way to get at a config's
{\tt DedispersionTree}s.) It is the algorithm behind {\tt pirate\_frb varmap df}.
...
{\tt GpuDetrenderLps2d} step omitted, so ``$A$'' here means the $A$ of
...
property is false with a {\tt GpuDetrenderLps2d} in front, so the day a detrender path is added is the
