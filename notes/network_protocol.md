...
- A zero-terminated ascii string follows, containing metadata in the format defined by
  ...
  Note that the metadata includes `freq_channels` and `beam_ids`. Here, `nbeams = len(beam_ids)` is
  the (receiver-dependent) number of  beams sent to the FRB search node, and `nfreq = len(freq_channels)`
  is the (sender-dependent) number of frequency channels sent by the X-engine node.
