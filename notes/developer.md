...
- A "minichunk" is 256 time samples. This is the cadence for sending data from the X-engine to the FRB backend (`struct Receiver`). It also sets the granularity of the per-(beam, freq, minichunk) scales/offsets in `struct AssembledFrame` -- and hence of the int4 -> float affine transform applied by `GpuDequantizationKernel`.
...
- A "frame" is a (chunk,beam) pair (not a (chunk,batch) pair!). Frames are used in `class MegaRingbuf`, and in the front-end server's intensity ring buffer (`FrbServer::frame_ringbuf`, whose `rb_*` counters are frame indices).
...
- **`SlabAllocator`**: A pool allocator. Carves fixed-size "slabs" from a
  `BumpAllocator` on demand (one per request, until the `BumpAllocator` is
  exhausted); slabs are recycled via reference counting -- when a slab's refcount
...
- **`FakeXEngine`**: A testing class that simulates upstream X-engine nodes sending data
  ...
  Used for end-to-end testing (e.g. `pirate_frb run fake_xengine`).
