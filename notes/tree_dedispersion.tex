...
The {\em exact} map ({\tt class PfAvarExact}) is treated in
\S\ref{ssec:svd_exact_map}--\S\ref{ssec:svd_exact_cost}, and is implemented in
\verb|pirate_frb/slow_avar/VarianceMapExact.py|. The linear algebra is
...
\S\ref{sec:analytic_variance} notationally (in particular $A$ is no longer transposed relative
...
of \S\ref{ssec:svd_exact_map}--\S\ref{ssec:svd_exact_cost} {\em is} implemented, in
