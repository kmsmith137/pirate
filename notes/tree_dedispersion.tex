...
\section{Downsampled trees and early triggers}
...
\item {\bf Downsampled trees} in order to search to higher DM. If we time-downsample the data by a factor $2^{\tt ids}$, and run rank-$r$ dedispersion as usual, then we can search to a max dispersion delay which is larger by a factor $2^{\tt ids}$. 
When we time-downsample by a factor $2^{\tt ids}$, we add time samples and multiply by $2^{-{\tt ids}/2}$, in order to leave the variance unchanged (assuming uncorrelated samples).

The integer {\tt ids} is called the {\em downsampling level}, and is denoted in the code by either \verb|ids| or \verb|ds_level|.
If ${\tt ids} > 0$, then we only keep the upper half (in delay) of the dedispersion output, since the lower half was already searched at downsampling level $({\tt ids}-1)$.

\item {\bf Early triggers} in order to reduce triggering latency, especially at high DM. By default, we only trigger when the pulse arrives at the highest tree-freq (i.e.\ lowest radio frequency). To trigger earlier for some bursts, we also run parallel dedispersion trees over the lowest $2^{r-\delta}$ tree-freqs, where $\delta=1,2,\cdots$ is denoted {\tt delta\_rank} in the code. These trees trigger at lower tree-freq (by a factor $2^\delta$), i.e.\ higher radio frequency.

\end{itemize}
A {\tt Dedisperser} consists of one or more {\tt DedispersionTrees}, each identified by a $({\tt ids}, \delta)$ pair.
The ``base'' tree $({\tt ids}, \delta)=(0,0)$ does not downsample or trigger early.
Conceptually, each {\tt DedispersionTree} just runs ``vanilla'' rank-$(r-\delta$) tree dedispersion on the first $2^{r-\delta}$ tree-freqs, downsampled in time by $2^{\tt ids}$, and then drops the lower DM-half of the outputs if ${\tt ids} > 0$.\footnote{This description suggests that each tree is an independent computation run ``from scratch''. There are a lot of tricks in the code that reuse computations between trees, i.e.\ the {\tt Dedisperser} is faster than running the constituent {\tt DedispersionTrees} independently.}

{\sloppy
As a concrete example, Table~\ref{tab:chord_sb2_et} and Figures~\ref{fig:early_triggers}--\ref{fig:tree_segments} show the ten-tree family of the CHORD config {\tt chord\_sb2\_et.yml} ($r=16$, with four downsampling levels and six early triggers).
\par}
...
\toprule
{\tt ids} & $\delta$ & Full-band delay range (s) & DM range (pc\,cm$^{-3}$) & trigger (MHz) & latency range (s) \\
\midrule
...
\bottomrule
\end{tabular}
\caption{The ten {\tt DedispersionTree}s of the CHORD config {\tt chord\_sb2\_et.yml} (rank $r=16$; four downsampling levels; six early triggers). Each tree is identified by an $({\tt ids}, \delta)$ pair. The DM range and {\em full-band} dispersion delay range depend only on {\tt ids}. The trigger frequency depends only on $\delta$, and the latency range is $2^{-\delta} \times \mbox{(delay range)}$.
...
\caption{A visualization of early triggers, in radio frequency (right panel) or the ``tree-freq'' reparametrization used in the code (left panel). Trees with early triggers search the lower part of the tree-freq range, i.e.\ the high part of the radio frequency range. A tree with $\delta > 0$ triggers earlier (by a factor $2^\delta$) than the main tree. The plot assumes CHORD params (300--1500 MHz, $r=16$, $0\le\delta\le 3$).}
...
\caption{The ten {\tt DedispersionTrees} from Table\ \ref{tab:chord_sb2_et}, shown as line segments in the (DM, triggering latency) plane. For each tree $({\tt ids}, \delta)$, the value of {\tt ids} determines the DM range, and the value of $\delta$ determines the slope.}
...
r_{\rm top} &= {\tt toplevel\_rank} & \mbox{ (tree rank specified in toplevel config file; denoted $r$ in earlier sections)} \nn
\end{align}
and the following per-tree parameters:
\begin{align}
{\tt ids} &= {\tt ds\_level} & \mbox{ (downsampling level of tree)} \nn \\
\delta &= {\tt delta\_rank}  & \mbox{ (``earliness'' of trigger)} \nn \\
...
\subsection{Details of output arrays}

The dedispersion output consists of a pair of 2-d arrays (\verb|out_max|, \verb|out_argmax|)
for each tree, indexed by (dm,time). (In the code, there is also a beam axis, omitted in
the notes for simplicity). The per-tree array shape $({\tt ndm\_out}, {\tt nt\_out})$ is given by:
\begin{align}
{\tt ndm\_out} &= \begin{cases}
  2^{r_{\rm top} - \delta} / D_{\rm ds} & \mbox{ if } {\tt ids}=0 \\
  2^{r_{\rm top} - \delta - 1} / D_{\rm ds} & \mbox{ if } {\tt ids} \ge 1
\end{cases} \label{eq:ndm_out} \\
{\tt nt\_out} &= \frac{T_{\rm in}}{2^{\tt ids} T_{\rm ds}} \label{eq:nt_out}
\end{align}
...
d_{\rm lo} = \begin{cases}
  0 & \mbox{ if } {\tt ids}=0 \\
  2^{r_{\rm top} + {\tt ids} - 1} & \mbox{ if } {\tt ids}\ge 1
\end{cases}
\hspace{1.5cm}
d_{\rm hi} = 2^{r_{\rm top} + {\tt ids}}
\label{eq:dlo_dhi}
\end{equation}
To derive Eqs.\ (\ref{eq:ndm_out})--(\ref{eq:dlo_dhi}), we walk through steps in the code:
\begin{itemize}

\item On the time axis, we start with a length-$T_{\rm in}$ input array,
  downsample by a factor $2^{\tt ids}$, run tree dedispersion,
...
\item The DM axis is a bit more complicated. First consider the base tree (no downsampling
  ...
  \end{equation}
  Then we include addiitonal complications one at a time.
  If we downsample the input by $2^{\tt ids}$, then $d_{\rm hi}$ gets multiplied by $2^{\tt ids}$.
  Recall that for ${\tt ids} > 0$, we also discard the lower half of the output DMs (which was already searched at downsampling level ${\tt ids}-1$).
  Therefore:
  \begin{equation}
   d_{\rm lo} = \begin{cases}
        0 & \mbox{ if } {\tt ids} = 0 \\
        2^{r_{\rm top} + {\tt ids} - 1} & \mbox{ if } {\tt ids} > 0
   \end{cases}
     \hspace{1cm}
   d_{\rm hi} = 2^{r_{\rm top} + {\tt ids}}
     \hspace{1cm}
   {\tt ndm\_out} = \begin{cases}
     2^{r_{\rm top}} & \mbox{ if } {\tt ids} = 0 \\
     2^{r_{\rm top}-1} & \mbox{ if } {\tt ids} > 0 \\
  \end{cases}
  \end{equation}
  If this is an early-triggered tree with $\delta > 0$, then the {\em full-band} delay
  range is unchanged, but the spacing between trial DMs increases by $2^\delta$, so
  ...
  \begin{equation}
   ...
   d_{\rm hi} = 2^{r_{\rm top} + {\tt ids}}
     \hspace{1cm}
   {\tt ndm\_out} = \begin{cases}
     2^{r_{\rm top}-\delta} & \mbox{ if } {\tt ids} = 0 \\
     2^{r_{\rm top}-\delta-1} & \mbox{ if } {\tt ids} > 0 \\
  \end{cases}
  \end{equation}
...
For a tree without an early trigger (i.e.\ $\delta=0$), we convert {\tt idm} and {\tt it} to $d$ and $t$ 
as follows:
\begin{equation}
...
If the tree has an early trigger (i.e.\ $\delta>0$), then there is an extra subtlety.
...
to decode it.)}
\begin{equation}
t' = t + \big( 1 - 2^{-\delta} \big) \, d \label{eq:idm_it_early}
\end{equation}
