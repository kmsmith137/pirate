...
\subsection{The out\_argmax array}
...
The method \verb|FrbGrouper.create_events()| decodes tokens to recover the ``winning''
parameters (min/max freq, DM, timestamp, width) for each \verb|out_max| array value.
A future version of these notes may explain details of the decoding.

For now, we just note one key property: the timestamp is defined to be the
arrival time at the lowest frequency of the band (400 MHz in CHIME, or 300 MHz 
...
In any case, the timestamp can be {\bf slightly earlier} than the beginning of
the time-chunk, due to the finite width of the peak-finding kernels.
