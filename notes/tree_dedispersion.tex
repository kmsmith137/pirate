...
working set: the banded factor plus window moments plus assembled matrix is about $2.8$ KB per (beam, time) in
float32 at $n_\phi = 2$, $n = 2$, $K \sim 16$, which is far past per-thread registers, so the natural
one-thread-per-(beam,time) mapping is not available at $n = 2$.
