...
Our goal is to find a low-rank, efficiently computable, accurate approximation
to the variance map $A$. Note that $A$ has many parameters: all the parameters
needed to configure tree dedispersion, and to configure a {\tt Detrender2d}.

In this section, let's brainstorm ideas, without implementing anything yet.
Some of these ideas are human-generated, others are LLM-generated.
...
\item As a heuristic approach, we might try modelling the effect of the {\tt Detrender2d}
as a DM-dependent time convolution. I think that the approach from the appendices generalizes
straightforwardly to the case where a time convolution is applied to the data -- but the 2-d
detrender is harder.

\end{itemize}

\subsection{Agentic approach (placeholder section)}
\label{ssec:agentic}

In this section, I plan to write about an agentic approach to exploring this problem.
