...
$n_j = {\tt nf\_tj}$.}
Flattening $y_j$ to a length-$2^{r-L} P$ row vector, we can therefore write
\begin{equation}
y_j = v^T \bar A,  \label{eq:abar_def}
\end{equation}
where $v$ is restricted to the contributing channels, and $\bar A$ is an $(n_j,\ 2^{r-L} P)$
matrix depending on $(\mbox{tree}, j)$. For CHORD-sized configs, $n_j$ is a few thousand and
$2^{r-L} P \sim 10^4$, so we never materialize $\bar A$; instead we construct a low-rank
factorization of it.
...
arrays into a matrix $A_\delta$ of shape $(n_{j\delta},\ 2^{{\rm popc}(\delta)} P)$, where
$n_{j\delta}$ is the number of contributing (channel, term) pairs,\footnote{Code name:
$n_{j\delta} = {\tt nf\_tjd}$.} and let $v_\delta$ denote the corresponding length-$n_{j\delta}$
slice of $v$. Then
\begin{equation}
y_j = \sum_\delta \big( v_\delta^T A_\delta \big) \, E_\delta,  \label{eq:group_sum}
\end{equation}
where $E_\delta$ is the $(2^{{\rm popc}(\delta)} P) \times (2^{r-L} P)$ zero/one ``expansion''
...
Empirically, $K_{\rm tot} \sim 30$--$160$ -- already far smaller than $n_j \sim 300$--$3000$.
...
Given an updated $v$, reevaluating the variances is two small matrix-vector multiplies and a
diagonal scaling,
\begin{equation}
y_j = \big( ( U_0 \, v ) \odot S_0 \big)^T \, V_0,
\end{equation}
i.e.\ $O\big(K_0 (n_j + 2^{r-L} P)\big)$ flops per $(\mbox{tree}, j)$; likewise the storage is
...
16$--$69$, with $\sum K_0 = 4969$ over all pairs versus $\sum K_{\rm tot} = 11301$ (a further
