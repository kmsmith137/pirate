...
\paragraph*{The shape of the approximation: coarse assignment.}
...
\end{equation}
To fix the shapes: $W$ is $N_{\rm freq}$-by-$K$, so its $K$ columns are frequency vectors (the
``dictionary''); $q_\beta$ is a column vector of $K$ nonnegative numbers; $W q_\beta$ is
...
nonnegative combination of the same $K$ dictionary vectors}.

This is the notes' own coarse-graining of \S\ref{ssec:first_milestone} with one generalization:
instead of assigning each group a single dictionary vector, we let it use a nonnegative
...
\paragraph*{Step 1: the dictionary (this is also the initialization).}
...
repeatedly merge the pair whose merge increases the objective least. The dictionary vector for
...
{\em Why shapes and not rows.} If a group is assigned a single dictionary vector, it is always
...
scale-invariant in each atom. Note this dictionary needs only $\bar A$ -- not even $y$.

{\em Why this is a good enough starting point.} The initial $W$ is a max envelope, so the
one-hot assignment $q_\beta = e_{c(\beta)}$ is feasible: the alternation starts from an
admissible point with a known finite $D$, and every step below preserves feasibility. The
dictionary also does not have to be optimal, only reasonable: building it from $\bar A$ at the
same coarse-graining used for assignment is within 3.6\% of building it from a finer $\bar A$,
and within 3.5\% of building it from the dense $A$. So there is no reason to keep a finer
$\bar A$ around -- the memory footprint is set by the assignment alone.

\paragraph*{Step 2: the Q-step (optimize the assignment, holding the dictionary fixed).}
...
heuristic -- given $W$, no better assignment exists. It uses only that $f$ is increasing, so it
...
\paragraph*{Step 3: the W-step (optimize the dictionary, holding the assignment fixed).}
...
the whole coarse-assigned family at any rank, and it is cheap to evaluate. Empirically the
...
found, which is the same greedy agglomerative merge in shape space but with one-hot assignment.

One quantity in the table needs defining first. Write ${\rm nnz}$ for the number of {\em nonzero}
entries of $q_\beta$: how many of the $K$ dictionary vectors a group actually uses. It is a
separate budget from the rank -- $K$ is the size of the {\em shared} dictionary, ${\rm nnz}$ is
how much of it each group draws on -- and the two enter the cost differently (see below).
The plain envelope is ${\rm nnz} = 1$ by construction, since each group is assigned one vector.
...
many dictionary vectors each group may use. Constraining ${\rm nnz}$ to 2 costs 14--34\% in $D$
...
\item {\bf It is cheaper to apply, not just more accurate.} Evaluating $y = Q(W^Tv)$ costs
 $K N_{\rm freq} + N_\beta\,{\rm nnz}$ multiply-adds, where ${\rm nnz}$ is the number of dictionary
 ...
 saving comes from coarse assignment: with a {\em per-row} $q_\alpha$ the same factorization is
...
\item {\bf The output is the array the kernel already stores.} \verb|PeakFindingKernelParams|'
 ...
 weight multiply twice and adds. A per-{\em row} assignment is not expressible in the current
...
\item Nothing at CHORD scale has been measured. The streaming reduction removes the memory
 ...
 accepts $(\bar A, y, \mbox{labels})$ would close this, since for a coarse-assigned
...
\item The shape-space greedy dictionary is the one step that does not scale:
...
\item Combining ideas from the previous two bullet points, we could also try 
 ...
 clustering algorithm above (``Step 1: the dictionary'').
...
\item {\bf Rank reduction guided by $D$ rather than by singular value.}
 ...
 which is the wrong norm here: campaign 1 found that pruning a dictionary by column {\em
...
\item {\bf Pivoted QR (or CUR) for choosing dictionary atoms.}
 Campaign 1 tried using actual rows of $A$ as dictionary atoms, chosen worst-covered-first,
 ...
 the pivot order gives a natural nested sequence of dictionaries -- which is exactly what a
 rank schedule wants.
...
\item {\bf Does a dictionary transfer between trees and between configs?}
 The dictionary build is the one step of the recommended algorithm that does not scale:
 ...
 So: does a dictionary built for one tree work for another (same $N_{\rm freq}$, different
 rank or subband structure)? If it does, the expensive step amortizes over trees, and
 possibly over configs. There is some reason for optimism -- campaign 1 found the dictionary
 need only be {\em reasonable} rather than optimal (a cheap k-means dictionary matched the
 greedy at $K=32$). Note the related question of transfer across {\em granularity} within one
 tree is already settled: building the dictionary from $\bar A$ at the same $L$ used for
 assignment is within a few percent of building it from a finer $\bar A$, so there is no
...
$q_\beta$, and alternate with the $W$-step. What changed is the {\em dictionary} and the
{\em constraint set}:

\begin{enumerate}
\item {\bf The dictionary is now a truncated SVD of $\bar A$}, not an agglomerative clustering.
...
\item {\bf The $Q$-step constraint $q_\beta \ge 0$ is dropped.} It was never implied by the
 ...
 The $W$-step's $W \ge 0$ may be kept when the dictionary is nonnegative (it is inactive at the
...
\begin{quote}
{\bf The SVD as an {\em approximation} and the SVD as a {\em dictionary} are different objects.}
...
$D = \infty$. The same truncation used as the {\em dictionary} $W$, with the LP free to choose
...
that the two changes are coupled. A signed dictionary is unusable while $q_\beta \ge 0$ is
...
(measured: identical $D_0$ to 17 digits). Dropping the constraint is what made the SVD dictionary
available at all.
...
coarse-assigned approximation of {\em any} rank can beat the unrescaled max-envelope over the
...
rank} -- and the campaign was right to optimize rank until the new dictionary landed.
...
The clustering dictionary was $O(N_\beta^{2.4} N_{\rm freq})$ and extrapolated to days at CHORD.
The SVD dictionary is one Gram eigendecomposition: $34$~s against $\sim 2.5$~h at subscale, and
$120$--$730$~s against $\sim 44$~h at the production point. Everything else was already linear or
better. Separately, the dictionary turns out not to need {\em fitting} at all: one $W$-step takes
a dictionary built for a {\em different} tree, subband count or detrender to within $3$--$9\%$ of
a purpose-built one, and the analytic no-detrender factorization of \S\ref{sec:analytic_variance}
costs only $1.0$--$1.8\times$ with no clustering whatever. A {\em random} dictionary costs
