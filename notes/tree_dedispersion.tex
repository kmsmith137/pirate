...
\end{equation}
{\em monotone} in $|\omega|$ with no stopband ripple, against the $\sim\!20\%$ ripple that
\S\ref{sec:time_detrending}'s truncated polynomial kernel has near $\omega \approx 3\pi/2W$.  This is the main
statistical argument for the estimator.  The kernel at zero lag is
\begin{equation}
...
widest searched boxcar.  Finite $L$ perturbs all of this by $O(e^{-L/\ell})$; note that truncating the future
...
under which Cholesky is equivariant, and the quantity that matters is the equilibrated condition number
...
so it cannot underflow for any realistic gap.
