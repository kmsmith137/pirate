...
\subsubsection{Campaign 2 setup [human-authored]}
...
\item Coarse-graining $A \rightarrow \bar A$ is a good approximation in practice (in the
 sense that $D(A,\bar A)$ is small). It seems safe to assume that this should always be
...
\item Given a rank-$K$ approximation $A^{\rm approx} = Q W^T$, and $K' < K$,
 ...
 $K$-by-$K$ diagonal. Then, decrease the rank to $K'$ by by dropping the lowest 
 (least significant) SVD modes, obtaining $A^{\rm new} = Q_1 \Lambda_1 W_1$.
...
\item Another idea for initialization (in addition to the clustering algorithm or
 ...
 \S\ref{ssec:analytic_variance}, we have an exact low-rank decomposition $\bar A_0 = Q_0 W_0^T$.
...
\item A small idea that may help speed up the LP solver.
 ...
 Then we could solve the constraints by simply rescaling the solution by $(1+\epsilon)$ --
