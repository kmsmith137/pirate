...
\item {\bf Downsampled trees} in order to search to higher DM. If we time-downsample the data by a factor $2^p$, and run rank-$r$ dedispersion as usual, then we can search to a max dispersion delay which is larger by a factor $2^p$.
...
The integer $p$ is called the {\em primary tree index}, and is denoted in the code by \verb|primary_tree_index| (or the loop variable \verb|ipri|). Each value of $p$ defines a {\em primary tree} which searches a different DM range, ordered from low to high DM.
...
In the config file, each primary tree specifies an integer {\tt num\_early\_triggers} $= N \ge 0$, which selects early triggers $\delta = 1, \ldots, N$.

\end{itemize}
A {\tt Dedisperser} consists of one or more {\tt DedispersionTrees}, each identified by a $(p, \delta)$ pair; i.e.\ each primary tree is expanded into $({\tt num\_early\_triggers}+1)$ {\tt DedispersionTrees}.
The ``base'' tree $(p, \delta)=(0,0)$ does not downsample or trigger early.
Conceptually, each {\tt DedispersionTree} just runs ``vanilla'' rank-$(r-\delta$) tree dedispersion on the first $2^{r-\delta}$ tree-freqs, downsampled in time by $2^p$, and then drops the lower DM-half of the outputs if $p > 0$.\footnote{This description suggests that each tree is an independent computation run ``from scratch''. There are a lot of tricks in the code that reuse computations between trees, i.e.\ the {\tt Dedisperser} is faster than running the constituent {\tt DedispersionTrees} independently.}

{\sloppy
As a concrete example, Table~\ref{tab:chord_sb2_et} and Figures~\ref{fig:early_triggers}--\ref{fig:tree_segments} show the ten-tree family of the CHORD config {\tt chord\_sb2\_et.yml} ($r=16$, with four primary trees and six early triggers).
\par}
...
\toprule
$p$ & $\delta$ & Full-band delay range (s) & DM range (pc\,cm$^{-3}$) & trigger (MHz) & latency range (s) \\
\midrule
...
\caption{The ten {\tt DedispersionTrees} from Table\ \ref{tab:chord_sb2_et}, shown as line segments in the (DM, triggering latency) plane. For each tree $(p, \delta)$, the value of $p$ determines the DM range, and the value of $\delta$ determines the slope.}
...
r_{\rm top} &= {\tt toplevel\_tree\_rank} & \mbox{ (toplevel tree rank, specified in the config file; denoted $r$ in earlier sections)} \nn
\end{align}
...
{\tt ndm\_out} &= \begin{cases}
  2^{r_{\rm top} - \delta} / D_{\rm ds} & \mbox{ if } p=0 \\
  2^{r_{\rm top} - \delta - 1} / D_{\rm ds} & \mbox{ if } p \ge 1
...
\item The DM axis is a bit more complicated. First consider the base tree (no downsampling
  ...
  \begin{equation}
   ...
   d_{\rm hi} = 2^{r_{\rm top} + p}
     \hspace{1cm}
   {\tt ndm\_out} = \begin{cases}
     2^{r_{\rm top}-\delta} & \mbox{ if } p = 0 \\
     2^{r_{\rm top}-\delta-1} & \mbox{ if } p > 0 \\
  \end{cases}
  \end{equation}
