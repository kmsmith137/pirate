...
The dedisperser's input is a shape $(N_{\rm freq}, N_{\rm time})$ array of channelized intensity time series, in which an FRB appears as a sweep with delay $\propto \nu^{-2}$.
The frequency channels need not be evenly spaced (in CHORD they are not).
Tree dedispersion (\S\ref{sec:tree_dedispersion}) searches for {\em straight lines}, so the first step is a change of variables on the frequency axis which makes the dispersion sweep straight.
This is the tree gridding kernel.
Its output is a shape $(N_{\rm tree}, N_{\rm time})$ array, where $N_{\rm tree} = 2^r$ and $r$ is the tree ``rank'' specified in the config file.
We index input frequency channels by $0 \le F < N_{\rm freq}$, and output ``tree-freq'' channels by $0 \le f < N_{\rm tree}$.
The time axis is a spectator: the kernel applies the same linear map to every time sample.

\par\noindent
{\bf Tree-freq channels.}
Dispersion delay is linear in $\nu^{-2}$, so tree-freq channels are equally spaced in $\nu^{-2}$.
Writing $[\nu_{\rm lo}, \nu_{\rm hi}]$ for the band, the $f$-th tree-freq edge $\nu_f$ is defined by
\begin{equation}
\nu_f^{-2} = \nu_{\rm hi}^{-2} + \frac{f}{N_{\rm tree}} \left( \nu_{\rm lo}^{-2} - \nu_{\rm hi}^{-2} \right),
  \qquad 0 \le f \le N_{\rm tree},
\end{equation}
and tree-freq channel $f$ is the frequency interval $[\nu_{f+1}, \nu_f]$.
Note that $f = 0$ is the {\em top} of the band: tree-freq index decreases with frequency.
An FRB with positive dispersion (lowest frequencies arrive last) is therefore a line of {\em positive} slope in the tree array (highest tree-freq channels arrive last), which is the convention used in the next section.

\par\noindent
{\bf Rebinning.}
The gridding kernel is a linear rebinning: each tree-freq channel is a weighted sum of the frequency channels which overlap it,
\begin{equation}
{\rm out}[f] = \sum_F w_{fF}\, {\rm in}[F], \qquad w_{fF} \ge 0,
\end{equation}
where ${\rm in}[F]$ and ${\rm out}[f]$ are length-$N_{\rm time}$ time series.
The weights are specified through an auxiliary length-$(N_{\rm tree}+1)$ array \verb|channel_map|, whose entries $c_f$ are floating-point frequency-channel indices, in the coordinate where frequency channel $F$ is the unit interval $[F, F+1)$.
Tree-freq channel $f$ is defined to occupy the interval
\begin{equation}
I_f = \big[\, c_{f+1},\ c_f \,\big), \qquad 0 \le f < N_{\rm tree},
\label{eq:tg_If_def}
\end{equation}
and $w_{fF}$ is the length of its overlap with frequency channel $F$:
\begin{equation}
w_{fF} = \max\!\big(\,0,\ \min(c_f,\, F+1) - \max(c_{f+1},\, F)\,\big).
\end{equation}
Since the intervals $I_f$ tile $[0, N_{\rm freq}]$, a frequency channel lying inside $I_f$ is copied to tree-freq channel $f$ with weight $1$, and a frequency channel containing an edge $c_f$ is split between tree-freq channels $f$ and $f-1$, with weights summing to $1$.
Thus $\sum_f w_{fF} = 1$ for every $F$, and the kernel conserves total intensity.

Conceptually, $c_f$ is the position of tree-freq edge $\nu_f$ in frequency-channel units: if $\nu_f$ falls inside frequency channel $F$, then $F \le c_f \le F+1$.
Note that $c_f$ is monotone decreasing, and $c_0 = N_{\rm freq}$, $c_{N_{\rm tree}} = 0$.

\par\noindent
{\bf The split rule.}
So far, we have explained how the tree gridding kernel is defined in terms of $c_f$, but we have not explained precisely how $c_f$ is defined.
We next describe two reasonable methods for defining $c_f$, the ``simple'' method and the ``chime'' method.
The difference is unimportant in practice.
We currently use the chime method, so that pirate's gridding agrees with bonsai's to roundoff.
We may switch to the simple method in the future, since it's a little more intuitive, and takes fewer lines of code.

Suppose tree-freq edge $\nu_f$ falls inside frequency channel $F$, whose edges are $\nu_F^- < \nu_F^+$, and write
\begin{equation}
c_f = F + s_f, \qquad 0 \le s_f \le 1.
\label{eq:cf_split}
\end{equation}
By Eq.\ (\ref{eq:tg_If_def}), $s_f$ is the fraction of channel $F$ assigned to tree-freq channel $f$ (the part below the edge), the remaining $1 - s_f$ going to tree-freq channel $f-1$.
A frequency channel gives one number for its whole bandwidth, so choosing $s_f$ requires an assumption about how intensity is distributed inside the channel.
Two natural choices are
\begin{align}
\mbox{``simple'' method:} \qquad
  s_f &= \frac{\nu_f - \nu_F^-}{\nu_F^+ - \nu_F^-}
  & \mbox{(intensity uniform in $\nu$)} \label{eq:split_simple} \\
\mbox{``chime'' method:} \qquad
  s_f &= \frac{(\nu_F^-)^{-2} - \nu_f^{-2}}{(\nu_F^-)^{-2} - (\nu_F^+)^{-2}}
  & \mbox{(intensity uniform in $\nu^{-2}$)} \label{eq:split_chime}
\end{align}
The simple method splits in proportion to bandwidth; $c_f$ is then just the fractional channel index of $\nu_f$ ({\tt frequency\_to\_index()} in the code).
The chime method splits in proportion to dispersion delay, and is the rule used by bonsai, the CHIME dedisperser.
Neither is more correct than the other: they amount to assuming a within-channel spectral index of $0$ or $-3$ respectively, which the data cannot distinguish.
They differ by at most $\frac{3}{8}\,\Delta\nu/\nu$ per weight, i.e.\ $10^{-5}$--$10^{-4}$ for CHIME and CHORD channel widths: irrelevant for detection, but far above float32 roundoff.
...
{\tt PeakFindingKernel} takes $D_{\rm ds} = 2^{R+K}$ as the constructor argument
{\tt PeakFindingKernelParams::dm\_downsampling}; it reads a $(2^{r-R}, M)$ array and reduces
over the low $K$ bits of the first index itself. A {\tt CoalescedDdKernel2} instead fixes
$2^{R+K} = 2^{\tt rank1}$ from its own ranks, and asserts that the caller's
{\tt dm\_downsampling} agrees. See comments in code.}
...
h_{\lambda,2} &= [\,\underbrace{\xi_3,\dots}_{2^\lambda},\ \underbrace{1,\dots}_{2^\lambda},\ \underbrace{\xi_3,\dots}_{2^\lambda}\,] \nn \\
h_{\lambda,3} &= [\,\underbrace{\xi_4,\dots}_{2^\lambda},\ \underbrace{1,\dots,\dots,1}_{2^{\lambda+1}},\ \underbrace{\xi_4,\dots}_{2^\lambda}\,]
\end{align}
The {\em tapers} $\xi_3$ and $\xi_4$ are the code constants {\tt constants::pf\_xi3} and
{\tt constants::pf\_xi4}, currently $1/3$ and $1/2$. These are the values used by bonsai, the
CHIME dedisperser, whose unnormalized profiles are $[1,3,1]$ and $[1,2,2,1]$.
The subscript is the kernel's width in blocks,
not its $q$ index: $\xi_3$ tapers the 3-block kernel $h_{\lambda,2}$, and $\xi_4$ the 4-block
kernel $h_{\lambda,3}$. We require $0 \le \xi_3, \xi_4 \le 1$, so that each kernel remains a
non-negative sum of co-centered boxcars (a hypothesis of the variance-map monotonicity result
in \vmnotes{Monotonicity of the variance map in the DM bits (no detrender)}).
...
Finally, a comment on subbands.
The subbands of \S\ref{sec:subband_search} are declared once, in the config file, and apply to trees $(\gamma,0)$ without early triggers (i.e.\ $e=0$).
For early trees $(e > 0)$, we truncate the counts array $(C_0, \cdots, C_R) \rightarrow (C_0, \cdots, C_{R-e})$, then clamp each surviving $C_l$ to its
maximum allowed value (see Eq.\ (\ref{eq:cl_max})).
Intuitively, early trees search all subbands which ``fit'' in the tree's frequency range.
A consequence of this rule: if early triggers are specified, then subbands must also be specified (more precisely, $C_{R-e}$ must be $>0$).
...
(Generated with {\tt pirate\_frb show dedisperser -v configs/dedispersion/chord\_sb2\_et.yml}.)}
...
\big(\mbox{32-bit {\tt argmax\_token}}\big)
 = \, \mbox{\tt (t) | (p << 8) | (m << 16) | (mu << 24)}
\end{equation}
i.e.~one byte per field, where $0 \le t < T_{\rm ds}$ is a fine-grained arrival time,
$0 \le p < P$ is a peak-finding profile index (encoding pulse width and profile shape),
$0 \le m < M \le 114$ is a ``multiplet'' (i.e.~(frequency subband, fine-grained DM) pair),
and $0 \le \mu < 2^K \le 16$ is part of the fine-grained DM as defined in the previous
paragraph. Note that $m$ and $\mu$ occupy separate bytes, so decoding a token does not
require knowing $K$.
...
the count of ``extra DM'' bits folded into the argmax token. The pipeline carries it only
through {\tt dm\_downsampling} $= 2^{R+K}$ ({\tt DedispersionTree} and
{\tt PeakFindingKernelParams}); {\tt ReferencePeakFindingKernel::K} derives it.
The kernel is emitted by {\tt cuda\_generator/SbDedisperser.py} and registered by
{\tt GpuSbDedispersionKernel} ({\tt include/pirate/DedispersionKernel.hpp});
{\tt ReferenceTree} is the CPU reference.
