...
\begin{equation}
\DD(r_1) = \big( \DD(R) \otimes 1 \big) \circ \LAG\big[ (2^R-1-F)\, e \big]
             \circ \big( 1 \otimes \DD(K) \big),
\end{equation}
where $0 \le e < 2^K$ is the delay produced by the inner $\DD(K)$, and the second stage's
frequency index $0 \le f_1 < 2^{r_1}$ splits as $f_1 = 2^K F + u$ with $0 \le u < 2^K$.
The coarse delay is now $d_{\rm hi} = 2^K d' + e$, so the lag the extraction needs splits
into two pieces:
\begin{equation}
(2^R-1-F)\, d_{\rm hi} = \underbrace{2^K (2^R-1-F)\, d'}_{\rm free}
                        + \underbrace{(2^R-1-F)\, e}_{\rm extra}.
\end{equation}
...
involve $d'$ at all, so it is one fixed lag per $(F,e)$ pair (a compile-time constant, in
...
$0 \le e < 2^K$ is part of the fine-grained DM as defined in the previous paragraph,
