...
Audit -- the second of these should return only the documented exceptions below
(the PfVariance progress dots, and `print(...)` inside docstring usage examples).
The first does NOT come back empty: it currently reports 114 sites. Every
`src_lib/*.cpp` has `using namespace std;`, so the grep must not anchor on the
`std::` prefix -- an earlier version of this audit did, always returned zero, and
so silently reported compliance.

Of those 114, 92 are single-threaded dump paths that are deliberately outside
the funnel (see "C++ dump-path exceptions" below). Of the 22 in threaded code,
9 already build a `stringstream` and emit it with a single `cout << ss.str()`,
which is atomic in practice -- including `HwtestSender::worker_main`, cited as
a model elsewhere in this file. That leaves 13 genuine chained-`operator<<`
sites in threaded code, which should be converted:

  - `FrbServer::_processing_thread_main` (8) -- the config-overwrite
    announcements and the `fill_analytic_weights()` progress lines.
  - `FileWriter::_try_to_delete_from_ssd` (3) and `_update_rpc_subscribers` (1).
  - `Hwtest::_show_stats` (1).

```
grep -rnE "\b(std::)?(cout|cerr)\b|\bprintf\(" src_lib/ | grep -vE "autogenerated_kernels|Casm"
grep -rn "\bprint(" pirate_frb/ --include=*.py | grep -vE "cuda_generator|/casm/"
```

(The `\b` is what keeps `atomic_print(` and `get_gmem_footprint(` out of the
second grep's results.)

The C++ audit covers `src_lib/` (where the threads are), not `include/`.
Headers hold two patterns that are deliberately outside the funnel: `show()`
/ `emit_cpp()` methods taking `std::ostream &os = std::cout` (the CALLER
supplies the stream, and these are used from single-threaded CLI dump paths),
and a one-shot `std::cout` note in `KernelRegistry::add()` that runs during
static registration. Routing those through `AtomicPrint` would mean pulling
`utils.hpp` into widely-included headers for no concurrency benefit.

Documented exceptions, all deliberate:

- **C++ dump-path exceptions** -- 92 of the 114 `src_lib/` hits, none of them
  in threaded code, and in each case the output IS the point of the command:
  the `test_*` / `time_*` unit-test and benchmark harnesses dispatched from
  `pirate_frb test` / `time` (57, including `ReferencePeakFindingKernel`'s
  token dumps), the one-off dev subcommands `revisit_512gb` and `scratch`
  (14), setup-time per-kernel `allocate()` progress (8), cleanup guards and
  `Socket::close` (5), and `make_random*` / `Params::print` config dumps (8).
  Routing these through `AtomicPrint` would buy nothing.
...
- `pirate_frb/slow_avar/PfVariance.py`: three `print(..., end="")` progress
  dots. Incremental partial-line output is fundamentally incompatible with
