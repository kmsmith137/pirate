...
ALL pirate output goes through one funnel, declared near the top of
...
Audit -- the first of these should come back empty; the second should return
only the documented exceptions below (the PfVariance progress dots, and
`print(...)` inside docstring usage examples):

```
grep -rn "std::cout\|std::cerr\|printf(" src_lib/ | grep -vE "autogenerated_kernels|Casm"
