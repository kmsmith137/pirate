...
```
Every run is seeded from a single master seed, so "randomized" does not mean "unreproducible".
`-s N` sets it (default `DEFAULT_SEED = 137`), `-r` draws one from OS entropy, and the seed is
printed at startup. `seed_rngs()` splits it into three streams -- `ksgpu::default_rng()` (the C++
side, including `DedispersionConfig::make_random()`), numpy's global random state
(`np.random.uniform()` and friends), and python's stdlib `random` -- and seeds once per process,
not per test, so iteration i draws different values from iteration j while the whole run replays.
The three streams are SHARED by all tests and consumed in test order, so replaying a failure
needs the same seed AND the same test flags AND the same `-n`. (`--net` and `--serv` replay only
partly: the sequence of operations `--net` issues depends on live server state, and anything
drawn on a spawned thread self-seeds. The test script spells out which parts replay when it
starts.)
...
3. Sometimes, reusable randomization logic needs to be "tweaked" in specific tests. For example, DedispersionConfig has a `dtype` field which is randomly either float32, or float16. A few of the tests require float32, so we introduced a boolean flag `force_float32=false` in DedispersionConfig::make_random(). (Over time, make_random() has grown to have ~10 arguments of this type, that are now in their own struct DedispersionConfig::RandomArgs.)
...
8. The `coverage` command. There is a CLI command `pirate_frb dev coverage` that is best explained by anecdote.
   ...
   The `pirate_frb dev coverage` command calls randomization utility functions and reports probabilities of particular events that are important in specific unit tests. For example, it calls DedispersionConfig::make_random() and reports how often the drawn config is float32, and how often max_width varies across primary trees. The report is organized by randomization function, and split into sections selected with `--config`, `--reg`, `--varmap` and `--dt` (default: all); `dev coverage -s X` multiplies every draw count, for when a rate sits near a band edge. We run `pirate_frb dev coverage` by hand occasionally to make sure that we aren't undersampling any important case. Nothing asserts, but every row carries a deliberately wide expected band, and the run ends with an "N rows, M outside band" line that names any row that moved -- a tripwire against a structure becoming unreachable, not a claim about a particular number.

   If you write a unit test where a specific type of random event is important, then you may want to add code to `pirate_frb dev coverage` to monitor the probability, in the future as code evolves. If you find yourself sampling and inspecting the output of a randomization function, or adding code to a test to check that a particular case is sampled, this is probably a sign that you should extend `pirate_frb dev coverage`. (You may need to refactor your randomization logic into its own function, so that it's callable from `coverage`.)
...
9. We try to design tests so that running for 100 iterations is a reasonable default that tests all code paths with high probability. Sometimes the best way to do this is by putting a short "inner" loop inside the main length-niter outer loop.
   ...
   Here's a specific example that occurs often. The `pirate_frb` package includes precompiled kernels (for example it might contain N=80 "cdd2" kernels, a specific type of kernel). The unit test CoalescedDdKernel2.test_random() chooses a random kernel to test (as part of more general randomization logic). Our high-level test script looks like this:
   ```py
    for i in range(niter):  # niter specified on command line (default 100)
        # Ensure that each kernel is run ~10 times (in expectation value) in a full run
        # (niter=100). rrange() is the shared helper for this: it does the (N+9)//10
        # arithmetic, and skips the test with a message if the registry is empty.
