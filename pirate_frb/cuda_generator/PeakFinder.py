...
class PeakFinder:
    ...
    INDICES. The peak-finder searches (multiplet, extra DM) pairs:
        ...
        m_ext  this class's PAIR index, m_ext = (m << K) | mu, 0 <= m_ext < M_ext = fs.M << K

    (Notation: M is always the compact multiplet count fs.M, and M_ext = M << K the pair
    count; likewise m is always a compact index and m_ext a pair index.)

    Registers and lanes are a flat resource, so the register-level methods below (pfx_*,
    pfy_*, pfz_*, the weight reads) work with m_ext throughout; the emitted register names
    (pfx_m{m_ext}..., pfz_m{m_ext}_p{p}, token_m{m_ext}_base) carry the pair index. Only two
    places know that m_ext is a pair:

      - process_pf_input(k, expr, m, mu) packs (m, mu) into m_ext;
      - _m_field_expr() unpacks m_ext into the argmax token's two bytes, m at bit 16 and mu
        at bit 24 (see PeakFindingKernel.hpp for the token format).
    ...
    so a run of 2^K consecutive m_ext -- the 2^K extra DMs of one multiplet -- share a weight,
    n(m_ext) = fs.m_to_n[m_ext >> K]. The band geometry (n_to_flo/n_to_fhi/m_to_d) is never
    touched. PfWeightReader takes K and applies that lookup itself.

    K = 0 (the default) means m_ext = m and mu is always zero.
    """
    ...
    def __init__(self, dtype, frequency_subbands, Wmax, Dcore, Dout, Tinner, ringbuf=None, K=0):
        ...
        assert utils.is_power_of_two(Wmax)
        ...
        self.M_ext = frequency_subbands.M << K      # PAIR count, see the class docstring

        self.weight_reader = PfWeightReader(frequency_subbands, dtype, Dcore, self.P, Tinner, K)
        self.weight_layout = self.weight_reader.weight_layout   # depends only on (N, P, Tinner, dtype)
        self.output = PfOutput(dtype, Dout)
        
        assert self.M_ext == self.weight_reader.M_ext
        ...
        # Typical kernel name: pf_fp32_n11_n6_n3_n1_K0_W16_Dcore8_Dout16_Tinner1
        self.kernel_name = f'pf_{dtype.fname}_{frequency_subbands.fstr}_K{K}_W{Wmax}_Dcore{Dcore}_Dout{Dout}_Tinner{Tinner}'
    ...
    def _emit_pf_constants(self, k):
        """Emits the profile tapers used by the p = 3l+2 and p = 3l+3 expressions.

        Called by CoalescedDdKernel2 as well, since a fused kernel splices in the same
        expressions. Emits the C++ names rather than their values, so that changing
        constants::pf_xi{3,4} leaves every autogenerated .cu file byte-identical.
        At Wmax == 1 there are no levels, and nothing would reference them.
        """

        if self.Wmax > 1:
            k.emit(f'const {self.dt32} pf_xi3 = {self.dtype.from_float("constants::pf_xi3")};')
            k.emit(f'const {self.dt32} pf_xi4 = {self.dtype.from_float("constants::pf_xi4")};')
            k.emit()


    def emit_kernel(self, k):
        dt32, SW, Dcore, Dout, M_ext = self.dt32, self.dtype.simd_width, self.Dcore, self.Dout, self.M_ext
        ...
        k.emit()
        k.emit('// Each warp processes a shape (M_ext,nt_in) input array (one beam, one output DM),')
        k.emit(f'// where M_ext = 2^K * M = {self.M_ext} is the number of (multiplet, extra DM) pairs (K = {self.K}).')
        k.emit('// Launch with B blocks, {32,W,1} threads.')
        k.emit('//')
        k.emit('// in: shape (B*W, 2^K, M, nt_in), i.e. one (mu, m) block of M_ext rows per warp')
        ...
        k.emit(f'in += warp * M_ext * {nt_in32};                // shape (B*W, M_ext, nt_in)')
        ...
        # The warp's input block is (2^K, M, nt_in): row order (mu, m), whereas the pair index
        # is m_ext = (m << K) | mu. So the loads are permuted (a compile-time offset each), and
        # process_pf_input() is called in increasing m_ext, m outer and mu inner.
        M, K = self.frequency_subbands.M, self.K

        for m in range(M):
            for mu in range(1 << K):
                moff = mu * M + m
                t = f'({moff} * {nt_in32})' if (moff > 1) else nt_in32
                t = f'in[{t} + threadIdx.x]' if (moff > 0) else 'in[threadIdx.x]'

                k.emit()
                if K == 0:
                    k.emit(f'// Read {m=} from global memory')
                else:
                    k.emit(f'// Read (m={m}, mu={mu}) from global memory: input row mu, multiplet m')
                self.process_pf_input(k, t, m, mu)
    ...
    def _emit_registration_boilerplate(self, k):
        ...
        k.emit(f'xassert_eq(fs.M << {self.K}, {self.M_ext});   // pair count M_ext')
        k.emit()
    ...
    def process_pf_input(self, k, expr, m, mu):
        ...
        (m, mu) into the pair index m_ext = (m << K) | mu (see the class docstring); calls
        must arrive in increasing m_ext, since pfx_register_ready() builds its transposes
        incrementally.
        """
        ...
        M_ext = self.M_ext
        Mpad = utils.align_up(M_ext, self.Dcore)

        b = tuple(('t',i) for i in range(self.pfx_nb))
        l = tuple(('t',i+self.pfx_nb) for i in range(self.pfx_nl))
        var = self.pfx_name(m_ext, 0, b, l)

        k.emit(f'{self.dt32} {var} = {expr};')
        self.pfx_register_ready(k, m_ext, 0, b, l)

        if (m_ext == M_ext-1) and (M_ext < Mpad):
            k.emit()
            k.emit(f'// FIXME: creating dummy pfx registers for M_ext <= m_ext < Mpad, where {M_ext=} and {Mpad=}.')
            k.emit(f'// FIXME: this is suboptimal but convenient!')

            for mm in range(M_ext, Mpad):
    ...
    def pfx_name(self, m_ext, t, b, l):
        """Helper for pfx_register_ready() and friends."""
        assert isinstance(m_ext,int) and isinstance(t,int)
        self._assert_is_si_tuple(b, self.pfx_nb)
        self._assert_is_si_tuple(l, self.pfx_nl)
        return f'pfx_m{m_ext}t{t}{self._si_tuple_str(b)}{self._si_tuple_str(l)}'

        
    def pfx_register_ready(self, k, m_ext, t, b, l):
        dt32, M_ext, Dcore = self.dt32, self.M_ext, self.Dcore
        ...
        for i in range(self.pfx_nl):
            ...
            lnew = l[:i] + (('m',i),) + l[(i+1):]
            iname0 = self.pfx_name(m_ext-mbit, t, b, l)
            iname1 = self.pfx_name(m_ext, t, b, l)
            oname0 = self.pfx_name(m_ext-mbit, t, b, lnew)
            oname1 = self.pfx_name(m_ext-mbit, t+tbit, b, lnew)
            ...
            self.pfx_register_ready(k, m_ext-mbit, t, b, lnew)
            self.pfx_register_ready(k, m_ext-mbit, t+tbit, b, lnew)
            return
        ...
        if (self.pfx_nb == 1) and (b[0] == ('t',0)):
            ...
            iname0 = self.pfx_name(m_ext-mbit, t, b, l)
            iname1 = self.pfx_name(m_ext, t, b, l)
            oname0 = self.pfx_name(m_ext-mbit, t, bnew, l)
            oname1 = self.pfx_name(m_ext-mbit, t+1, bnew, l)
            ...
            self.pfx_register_ready(k, m_ext-mbit, t, bnew, l)
            self.pfx_register_ready(k, m_ext-mbit, t+1, bnew, l)
            return

        # If we get here, then register should be "all ms".
        assert (m_ext % Dcore) == 0
        assert b == tuple(('m',i+self.pfx_nl) for i in range(self.pfx_nb))
        assert l == tuple(('m',i) for i in range(self.pfx_nl))

        if (t == Dcore-1):
            k.emit()
            k.emit(f'// Transpose complete -- now ready to run peak-finders for {m_ext} <= m_ext < {m_ext+Dcore}')
            k.emit(f'// First, some renames for readability.')

            for tt in range(Dcore):
                k.emit(f'{dt32} {self.pfy_name(1,m_ext,tt)} = {self.pfx_name(m_ext,tt,b,l)};')

            self.pfy_registers_ready(k, m_ext)


    def pfy_name(self, w, m_ext, t):
        """Helper for pfy_registers_ready() and friends."""
        return f'pfy{w}_m{m_ext}_t{t}' if (t >= 0) else f'pfy{w}_m{m_ext}_tn{-t}'

    
    def _get_pfy(self, k, w, m_ext, t, allow_ringbuf=True):
        """Helper for pfy_registers_ready()."""

        dt32, Dcore, Minner = self.dt32, self.Dcore, self.Minner
        
        # Convenient shorthand.
        pfy = lambda w,t: self.pfy_name(w,m_ext,t)
        check = lambda w,t: ((w,m_ext,t) in self.pfys_held)
        ...
        raise RuntimeError(f'Internal error: PeakFinder._get_pfy({w=}, {m_ext=}, {t=}, {allow_ringbuf=}) failed')

    
    def _read_weights(self, k, m_ext, p):
        ...
        if self.dtype == 'float':
            assert Dcore == Minner
            self.weight_reader.read_weights(k, m_ext, p)
        
        elif self.dtype == '__half' and ((p % 2) == 0):
            assert Dcore == 2*Minner
            self.weight_reader.read_weights(k, m_ext, p)
            self.weight_reader.read_weights(k, m_ext+Minner, p)
                
        elif self.dtype != '__half':
            raise RuntimeError('should never get here')

    
    def _absorb_pfz(self, k, var, m_ext, p, t):
        """Absorb pfz values into the running max.

        Helper for pfy_registers_ready().
        Variables named 'pfz_m{m_ext}_p{p}' store max-values (either float or __half2)
        Variables named 'pfiz_m{m_ext}_p{p}' store "mini-tokens" 0 <= t < Dcore.
        Mini-tokens are always declared 'uint', but are secretly either u32 or u16x2.
        """
        
        pfz = f'pfz_m{m_ext}_p{p}'
        pfiz = f'pfiz_m{m_ext}_p{p}'
        assert 0 <= t < self.Dcore

        if (m_ext,p) not in self.pfz_decl:
            k.emit(f'{self.dt32} {pfz} = {var};')
            k.emit(f'uint {pfiz} = {t};')
            self.pfz_decl.add((m_ext,p))
    ...
    def pfy_registers_ready(self, k, m_ext):
        dt32, Wmax, Dcore, Minner = self.dt32, self.Wmax, self.Dcore, self.Minner
        assert m_ext % Dcore == 0
        ...
        for t in range(Dcore):
            self.pfys_held.add((1,m_ext,t))

        self._read_weights(k, m_ext, p=0)
        
        k.emit()
        k.emit(f'// Compute p=0 peak-finding kernel')
        for t in range(0, self.Dcore):
            self._absorb_pfz(k, pfy(1,t), m_ext, 0, t)

        self.pfz_register_ready(k, m_ext, 0)

        # Loop over w = 1, 2, 4, ..., (Wmax//2).
        for lgw in range(utils.integer_log2(Wmax)):
            w = 2**lgw
            ...
            for t in tstarts:
                self._get_pfy(k, w, m_ext, t)
                
            self._read_weights(k, m_ext, 3*lgw+1)

            k.emit()
            k.emit(f'// Compute p={3*lgw+1} peak-finding kernel.')
            for tend in tends:
                self._get_pfy(k, 2*w, m_ext, tend-2*w, allow_ringbuf=False)
                self._absorb_pfz(k, pfy(2*w,tend-2*w), m_ext, 3*lgw+1, tend-t0)
            self.pfz_register_ready(k, m_ext, 3*lgw+1)
            
            self._read_weights(k, m_ext, 3*lgw+2)
            
            k.emit()
            k.emit(f'// Compute p={3*lgw+2} peak-finding kernel.')
            for tend in tends:
                tmp = k.get_name('tmp')
                k.emit(f'{dt32} {tmp} = {pfy(w,tend-2*w)} + pf_xi3 * ({pfy(w,tend-3*w)} + {pfy(w,tend-w)});  // p={3*lgw+2}')
                self._absorb_pfz(k, tmp, m_ext, 3*lgw+2, tend-t0)
            self.pfz_register_ready(k, m_ext, 3*lgw+2)

            self._read_weights(k, m_ext, 3*lgw+3)
            self._get_pfy(k, 2*w, m_ext, tends[0]-3*w, allow_ringbuf=False)    # stray

            k.emit()
            k.emit(f'// Compute p={3*lgw+3} peak-finding kernel.')
            for tend in tends:
                tmp2 = k.get_name('tmp')
                k.emit(f'{dt32} {tmp2} = {pfy(2*w,tend-3*w)} + pf_xi4 * ({pfy(w,tend-4*w)} + {pfy(w,tend-w)});  // p={3*lgw+3}')
                self._absorb_pfz(k, tmp2, m_ext, 3*lgw+3, tend-t0)
            self.pfz_register_ready(k, m_ext, 3*lgw+3)

    
    def _m_field_comment(self):
        return '(m_ext << 16) | (mu << 24)' if self.K else '(m_ext << 16)'
    ...
    def pfz_register_ready(self, k, m_ext, p):
        """
        Called when the following vars are ready:

          - pfz_m{m_ext}_p{p}: stores max-values (either float or __half2)

          - pfiz_m{m_ext}_p{p}: store "mini-tokens" 0 <= t < Dcore.
            Mini-tokens are always declared 'uint', but are secretly either u32 or u16x2.

        Register assignment here is (where Lm = log2(Minner) and L = log2(Dcore); the m_i
        below are bits of the pair index m_ext, see the class docstring)
        ...
        Thus, each call to pfz_register_ready() processes (32*SW) times, (Dcore) m_ext-values,
        and 1 p-value.
        ...
        convert a minitoken to a token, we do something like this:
            ...
            me = m_ext | (threadIdx.x & (Minner-1));   // this lane's pair index
            me = min(me, M_ext-1);                     // the last lane group can overhang M_ext
            assert (p < P);                            // p is the same on all threads
            token = tfull | (p << 8) | _m_field_expr(me);
        """

        Dcore, M_ext, Minner, P = self.Dcore, self.M_ext, self.Minner, self.P
        assert (m_ext % Dcore) == 0
        
        k.emit()
        k.emit(f'// pfz_register_ready({m_ext=}, {p=}) called.')

        # "Base" registers (p == 0).

        # Note the min() clamps the pair index, before _m_field_expr() splits it into the
        # token's two bytes: the clamp exists because the last lane group can overhang M_ext,
        # which is a property of the lane layout rather than of the token format.

        if (self.dtype == 'float') and (p == 0):
            k.emit(f'// token_m{m_ext}_base = {self._m_field_comment()} | (t & (Dout-Dcore))')
            k.emit(f'uint token_m{m_ext}_base = {m_ext} | (threadIdx.x & (Minner-1));  // m_ext + mi')
            if m_ext + Minner >= M_ext:
                k.emit(f'token_m{m_ext}_base = min(token_m{m_ext}_base, M_ext-1);')
            mf = self._m_field_expr(f'token_m{m_ext}_base')
            k.emit(f'token_m{m_ext}_base = {mf} | (threadIdx.x & (Dout-Dcore));')
        
        elif (self.dtype == '__half') and (p == 1):
            for mm in [ m_ext, (m_ext+Minner) ]:
                k.emit(f'// token_m{mm}_base = {self._m_field_comment()} | (t & (Dout-Dcore))')
                k.emit(f'uint token_m{mm}_base = {mm} | (threadIdx.x & (Minner-1));  // m_ext + mi')
                if mm + Minner >= M_ext:
                    k.emit(f'token_m{mm}_base = min(token_m{mm}_base, M_ext-1);')
        ...
        if self.dtype == 'float':
            k.emit(f'pfz_m{m_ext}_p{p} *= pfw_m{m_ext}_p{p};')
            k.emit(f'uint token_m{m_ext}_p{p} = token_m{m_ext}_base | ({p} << 8) | pfiz_m{m_ext}_p{p};')
            self.output.apply_inner(k, f'pfz_m{m_ext}_p{p}', [ f'token_m{m_ext}_p{p}' ])

        elif (self.dtype == '__half') and ((p % 2) == 0) and (p < (P-1)):
            k.emit(f'// This is a no-op (need to wait for p={P+1}).')

        elif (self.dtype == '__half') and ((p % 2) == 0) and (p == (P-1)):
            k.emit(f'// Creating dummpy pfz registers for {m_ext=}, p={P}')
            k.emit(f'// FIXME: this is suboptimal but convenient!')
            k.emit(f'__half2 pfz_m{m_ext}_p{P} = pfz_m{m_ext}_p{P-1};')
            k.emit(f'uint pfiz_m{m_ext}_p{P} = pfiz_m{m_ext}_p{P-1};')
            self.pfz_register_ready(k, m_ext, P)

        elif (self.dtype == '__half') and (p % 2):
            m0, m1 = m_ext, (m_ext+Minner)
            p0, p1 = (p-1), p
            ...
            k.emit(f'// Input: {pfz0}, {pfz1}  (simd <-> (m_ext{m0},m_ext{m1}), reg <-> (p{p0},p{p1})')
            k.emit(f'// Output: {pfu0}, {pfu1} (simd <-> (p{p0},p{p1}), reg <-> (m_ext{m0},m_ext{m1})')
    ...
    def write_kernel(cls, filename):
        ...
        # Typical basename: pf_fp32_n11_n6_n3_n1_K0_W16_Dcore8_Dout16_Tinner1.cu
        m = re.fullmatch(r'pf_(fp\d+)_((?:n\d+_)*n\d+)_K(\d+)_W(\d+)_Dcore(\d+)_Dout(\d+)_Tinner(\d+)\.cu', basename)
        if not m:
        ...
        K, Wmax, Dcore, Dout, Tinner = (int(m.group(i)) for i in range(3, 8))

        pf_kernel = cls(dtype, frequency_subbands, Wmax, Dcore, Dout, Tinner, K=K)
...
class PfWeightReader:
    def __init__(self, frequency_subbands, dtype, Dcore, P, Tinner, K=0):
        """
        ...
        The index this class reads weights for is PeakFinder's pair index
        m_ext = (m << K) | mu, 0 <= m_ext < M_ext = frequency_subbands.M << K. The only thing
        K changes here is that 2^K consecutive m_ext share a subband:
        n(m_ext) = m_to_n[m_ext >> K] (see _get_I()).
        """

        self.dtype = dtype = Dtype(dtype)
        self.weight_layout = PfWeightLayout(frequency_subbands, dtype, P, Tinner)   # uses N only
        self.frequency_subbands = frequency_subbands
        self.Tinner = Tinner
        self.Dcore = Dcore
        self.P = P
        ...
        self.Pinner = self.weight_layout.Pinner
        self.K = K
        self.M_ext = frequency_subbands.M << K   # pair count, see docstring
        self.N = frequency_subbands.N
        assert K >= 0
        ...
        self.Mouter = Mouter = (self.M_ext + Minner - 1) // Minner

        # Typical kernel name: pf_weight_reader_fp32_n11_n6_n3_n1_K0_Dcore8_P13_Tinner2
        self.kernel_name = f'pf_weight_reader_{dtype.fname}_{frequency_subbands.fstr}_K{K}_Dcore{Dcore}_P{P}_Tinner{Tinner}'
        self.kernel_basename = self.kernel_name + '.cu'
    ...
    def read_weights(self, k, m_ext, p):
        """After calling this function, weights are available with variable name 'pfw_m{m_ext}_p{p}'."""

        k.emit(f'// PfWeightReader.read_weights({m_ext=}, {p=}): start.')
        wp, N, Minner, Pinner, Tinner = self.wp, self.N, self.Minner, self.Pinner, self.Tinner

        if p == 0:
            self._init_pfI(k, m_ext)
        ...
        Imin = min(self._get_I(m_ext+mi, p, 0) for mi in range(Minner))
        Imax = max(self._get_I(m_ext+mi, p, Tinner-1) for mi in range(Minner))
        ...
        k.emit(f'{self.dt32} pfw_m{m_ext}_p{p} = __shfl_sync(~0u, {wcl}, {Istr});')
        k.emit(f'// PfWeightReader.read_weights({m_ext=}, {p=}): end')


    def _get_I(self, m_ext, p, tinner):
        ...
        m_ext = min(m_ext, self.M_ext-1)
        n = self.frequency_subbands.m_to_nd[m_ext >> self.K][0]   # 2^K consecutive pair indices share a subband
    ...
    def _init_pfI(self, k, m_ext):
        ...
        pfI = f'pfI_m{m_ext}'
        Ilist = [ self._get_I(m_ext+mi,0,0) for mi in range(Minner) ]
        ...
        k.emit(f'//')
        k.emit(f'// Compute {pfI}: the I-index for (m_ext,p)=({m_ext},0), and (minner,tinner)')
        ...
        for mi in range(1, Minner):
            if Ilist[mi] != Ilist[mi-1]:
                k.emit(f'{pfI} = ({minner} < {mi}) ? {pfI} : {Ilist[mi]};')
    ...
    def emit_kernel(self, k):
        ...
        k.emit(f'out += Pouter * (threadIdx.x & (Minner-1));   // m_ext-offset')
        ...
        for m_ext in range(0, self.M_ext, self.Minner):
            for p in range(0, self.P, self.Pinner):
                self.read_weights(k, m_ext, p)
                pouter = utils.xdiv(p, self.Pinner)

                k.emit()
                k.emit(f'// (m_ext, p) = ({m_ext}, {p})')
                k.emit(f'out[({m_ext} * Pouter) + {pouter}] = pfw_m{m_ext}_p{p};')
                k.emit()
        ...
        k.emit('struct register_hack {')
        k.emit('register_hack() {')
        k.emit('PfWeightReaderMicrokernel::RegistryKey k;')
        k.emit(f'k.dtype = ksgpu::Dtype::native<{self.dtype.scalar}>();')
        k.emit(f'k.subband_counts = {{ {sb_counts} }};')
        k.emit(f'k.K = {self.K};')
    ...
    def write_kernel(cls, filename):
        ...
        # Typical basename: pf_weight_reader_fp32_n11_n6_n3_n1_K0_Dcore8_P13_Tinner2.cu
        m = re.fullmatch(r'pf_weight_reader_(fp\d+)_((?:n\d+_)*n\d+)_K(\d+)_Dcore(\d+)_P(\d+)_Tinner(\d+)\.cu', basename)
        if not m:
            raise RuntimeError(f"Couldn't match filename '{filename}'")

        dtype = Dtype(m.group(1))
        frequency_subbands = FrequencySubbands.from_fstr(m.group(2))
        K, Dcore, P, Tinner = (int(m.group(i)) for i in range(3, 7))

        pf_weight_reader = PfWeightReader(frequency_subbands, dtype, Dcore, P, Tinner, K)
...
class PfOutput:
    ...
    def write_kernel(cls, filename):
        ...
        k.emit('struct register_hack {')
        k.emit('register_hack() {')
        k.emit('PfOutputMicrokernel::RegistryKey k;')
