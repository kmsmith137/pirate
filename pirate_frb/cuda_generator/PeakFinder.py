...
class PeakFinder:
    ...
    PfWeightReader takes K and applies n(j) = m_to_n[j >> K] itself.

    K = 0 (the default) means j = m and mu is always zero.
    """
    ...
    def emit_kernel(self, k):
        ...
        k.emit(f'// where M = 2^K * Mc = {self.M} is the number of (multiplet, extra DM) pairs (K = {self.K}).')
        k.emit('// Launch with B blocks, {32,W,1} threads.')
        k.emit('//')
        k.emit('// in: shape (B*W, 2^K, Mc, nt_in), i.e. one (mu, m) block of M rows per warp')
        ...
        # The warp's input block is (2^K, Mc, nt_in): row order (mu, m), whereas the pair index
        # is (m << K) | mu. So the loads are permuted (a compile-time offset each), and
        # process_pf_input() is called in increasing pair index, m outer and mu inner.
        Mc, K = self.frequency_subbands.M, self.K
    ...
    def _emit_registration_boilerplate(self, k):
        ...
        k.emit(f'xassert_eq(fs.M << {self.K}, {self.M});   // pair count')
        k.emit()
...
class PfWeightReader:
    def __init__(self, frequency_subbands, dtype, Dcore, P, Tinner, K=0):
        ...
        The index this class reads weights for is PeakFinder's pair index j = (m << K) | mu,
        0 <= j < M = frequency_subbands.M << K. The only thing K changes here is that 2^K
        consecutive j share a subband: n(j) = m_to_n[j >> K] (see _get_I()). The 'm' in the
        method parameters below is that pair index.
        """
        ...
        self.Pinner = self.weight_layout.Pinner
        self.K = K
        self.M = frequency_subbands.M << K   # pair count, see docstring
        self.N = frequency_subbands.N
        assert K >= 0
    ...
    def _get_I(self, m, p, tinner):
        ...
        m = min(m, self.M-1)
        n = self.frequency_subbands.m_to_nd[m >> self.K][0]   # 2^K consecutive pair indices share a subband
    ...
    def _init_pfI(self, k, m):
        ...
        pfI = f'pfI_m{m}'
        Ilist = [ self._get_I(m+mi,0,0) for mi in range(Minner) ]
        ...
        k.emit(f'//')
        k.emit(f'// Compute {pfI}: the I-index for (m,p)=({m},0), and (minner,tinner)')
