...
class PeakFindingParams:
    def __init__(self, dtype, M, E, Dout, Dcore=None, W=None, BlocksPerSM=None):
        """
        Input compile-time params:
    ...
    @property
    def filename(self):
        return f'pf_fp{self.nbits}_M{self.M}_E{self.E}_Dout{self.Dout}.cu'

    
    @classmethod
    def from_filename(cls, filename):
        """
        Example filename: 'pf_fp32_M16_E8_Dout16.cu'.

        The filename can include an optional leading directory and a trailing '.cu'.
        For example, /path/to/pf_fp32_M16_E8_Dout16.cu matches.
        """
        ...
        try:
            ...
            m = re.match(r'^(?:.*/)?pf_(fp\d+)_M(\d+)_E(\d+)_Dout(\d+)(?:\.cu)?', filename)
            dtype = dt_dict[m.group(1)]
            M = int(m.group(2))
            E = int(m.group(3))
            Dout = int(m.group(4))
        except:
            raise RuntimeError(f"cuda_generator.PeakFindingParams: couldn't parse {filename=}")

        return cls(dtype=dtype, M=M, E=E, Dout=Dout)
        

####################################################################################################


def make_pf_file(filename):
    """Returns a Kernel object."""
    
    params = PeakFindingParams.from_filename(filename)
    pf_full_kernel = PeakFinder(params, reduce_only=False)
    pf_reduce_only = PeakFinder(params, reduce_only=True)
    ...
    k.emit(f'k.M = {params.M};')
    k.emit(f'k.E = {params.E};')
    k.emit(f'k.Dout = {params.Dout};')
    k.emit()
    
    k.emit('GpuPeakFindingKernel::RegistryValue v;')
    k.emit(f'v.Dcore = {params.Dcore};')
    k.emit(f'v.W = {params.W};')
    k.emit(f'v.P = {params.P};')
