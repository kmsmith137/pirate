...
def random_kernels(args):
    ...
    if args.cdd2:
        ...
        # min(Dout,8), both computed in makefile_helper.autogenerated_cdd2_kernels()'s emit
        # loop. See the "Dout INVARIANT" comment there. (The --pf branch below still prints
        ...
        for _ in range(args.n):
            ...
            # pf_rank <= dd_rank1 at the LARGEST dd_rank of the row, and must survive
            # restriction by every et_level emitted. makefile_helper enforces both and
            # raises at makefile-generation time; drawing them unconstrained here is what
            # put unbuildable rows into its debug list.
            ...
            for dd_rank in range(dd_rank_min, dd_rank_max+1):
                ...
                atomic_print(f"('fp{nbits}', {dd_rank}, {Wmax}, {Tinner}, {list(subband_counts)}, {et_levels}),{s}")
