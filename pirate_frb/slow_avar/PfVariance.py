...
def test_approximation(config):
    ...
    for m in range(M):
        ...
        if not var_exact.terms or not var_approx.terms:
            raise RuntimeError(f"test_approximation: empty PfVariance at m={m} "
        ...
        if (a_exact <= 0.0).any() or (a_approx <= 0.0).any():
            raise RuntimeError(f"test_approximation: non-positive variance at m={m}")
