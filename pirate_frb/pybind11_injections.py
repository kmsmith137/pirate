...
@ksgpu.inject_methods(pirate_pybind11.SinglePulse)
class SinglePulseInjections:
    ...
    def get_signal_to_noise(self, sample_rms=1.0, channel_weights=None):
        ...
        """
        nfreq = self.nfreq
        sample_rms = np.asarray(sample_rms, dtype=np.float64)
        cw = None if channel_weights is None else np.asarray(channel_weights, dtype=np.float64)
