"""Associate duplicate offline PIRATE candidates into representative events.
...
class GpuDecodedCandidates:
    ...
    same device as the GroupingGeometry tables.  The normalized schema produced
...
class GpuMemberTable:
    ...
    duplicated: index GpuGroupingResult.candidates with candidate_index.  Rows
...
def _require_current_grouping_device(geometry):
    ...
    if (isinstance(geometry, GroupingGeometry)
...
def _validate_geometry(geometry):
    ...
    if not isinstance(geometry, GroupingGeometry) or geometry.ntrees <= 0:
        raise TypeError("geometry must be a non-empty GroupingGeometry")
    tables = (
...
def _group_candidates_serial_oracle(decoded, geometry, *, config=None):
    ...
    representative loop.  Production calls use :func:`group_candidates`, whose
    ...
    geometry : GroupingGeometry
        ...
        same device.
    config : GroupingConfig or None, keyword-only
        Association tolerances; None uses GroupingConfig defaults.
