...
.PHONY: all lib build_wheel build_sdist clean docs docs-clean docs-serve pybind11 autogenerated_kernels gddk gpfk pfom pfwr cdd2 help
...
CLEAN_RMDIRS := lib pirate_frb/__pycache__ pirate_frb/casm/__pycache__ pirate_frb/core/__pycache__ pirate_frb/kernels/__pycache__ pirate_frb/loose_ends/__pycache__ pirate_frb/utils/__pycache__ pirate_frb/cuda_generator/__pycache__
...
# Convert both .cu and .cpp source files to .o object files
LIB_OFILES_CU := $(filter %.cu,$(LIB_SRCFILES))
LIB_OFILES_CPP := $(filter %.cpp,$(LIB_SRCFILES))
LIB_OFILES := $(LIB_OFILES_CU:%.cu=%.o) $(LIB_OFILES_CPP:%.cpp=%.o) $(AUTOGENERATED_OFILES)
...
DEPFILES := $(ALL_SRCFILES_CU:%.cu=%.d) $(ALL_SRCFILES_CPP:%.cpp=%.d)
...
$(PIRATE_LIB): $(LIB_OFILES) makefile_helper.out
	@mkdir -p lib
	$(NVCC) $(NVCC_ARCH) -shared -o $@ $(LIB_OFILES) -lksgpu -lyaml-cpp -L$(KSGPU_DIR)/lib -Xcompiler '"-Wl,-rpath=$(KSGPU_DIR)/lib"'
