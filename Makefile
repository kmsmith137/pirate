...
CLEAN_FILES := sdist_files.txt wheel_files.txt makefile_helper.out pirate_frb/include pirate_frb/lib $(AUTOGENERATED_KERNELS) $(GRPC_CCFILES) $(GRPC_HFILES) $(GRPC_PYFILES) grpc/*.protoc_*_sentinel asdf-cxx/include/asdf/config.hxx
...
# are the targets of a single (grouped) pattern rule. protoletariat (protol)
# then rewrites the imports to be relative within the package.
pirate_frb/rpc/grpc/%_pb2.py pirate_frb/rpc/grpc/%_pb2_grpc.py: grpc/%.proto
...
# __init__.py is created (as an empty file) as a side effect of
# `protol --create-package` in the rule above. If only this file is deleted
# (with the _pb2 stubs still up to date) that rule won't re-run, so give
# __init__.py its own touch recipe; it depends on $(GRPC_PROTO) so it is
# recreated whenever a proto changes.
