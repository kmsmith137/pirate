...
CLEAN_RMDIRS := lib pirate_frb/__pycache__ pirate_frb/casm/__pycache__ pirate_frb/chime/__pycache__ pirate_frb/core/__pycache__ pirate_frb/kernels/__pycache__ pirate_frb/loose_ends/__pycache__ pirate_frb/utils/__pycache__ pirate_frb/cuda_generator/__pycache__ pirate_frb/rpc/__pycache__ pirate_frb/rpc/grpc/__pycache__
...
# __init__.py is created (as an empty file) as a side effect of `protol --create-package`
# in the sentinel recipe above -- but if only this file is deleted (sentinel still up to date),
# an empty-recipe rule wouldn't re-trigger the sentinel. Give it its own touch recipe so make
# can recreate it independently. Hardcoded (no % stem); depends on $(GRPC_SENTINELS) so it's
# rebuilt if any proto changes.
pirate_frb/rpc/grpc/__init__.py: $(GRPC_SENTINELS)
	touch $@
