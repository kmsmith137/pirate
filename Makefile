...
CLEAN_DIRS := . lib src_bin src_lib src_lib/loose_ends src_lib/template_instantiations src_lib/autogenerated_kernels src_pybind11 pirate_frb pirate_frb/__pycache__ pirate_frb/cuda_generator/__pycache__ include include/pirate include/pirate/loose_ends include/pirate/cuda_kernels
...
CLEAN_RMDIRS := bin lib pirate_frb/__pycache__ pirate_frb/cuda_generator/__pycache__

blah:
	echo $(CLEAN_FILES)
	echo $(wildcard $(CLEAN_FILES) $(BIN_XFILES))
...
ALL_SRCFILES := $(LIB_SRCFILES) $(PYEXT_SRCFILES) $(BIN_SRCFILES) $(AUTOGENERATED_KERNELS)
DEPFILES := $(ALL_SRCFILES:%.cu=%.d)

# Note that autogenerated .cu files are not included in the sdist.
SDIST_FILES := pyproject.toml Makefile makefile_helper.py autogenerate_kernel.py
SDIST_FILES += $(PYFILES) $(CUDAGEN_PYFILES) $(LIB_SRCFILES) $(PYEXT_SRCFILES) $(BIN_SRCFILES) $(HFILES)
