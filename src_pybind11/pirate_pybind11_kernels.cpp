...
// Docstring shared by the six registry_keys() bindings below.
//
// A registry is a static table built at load time by the autogenerated kernels, so what it
// holds is a property of what this build COMPILED, not of any draw. 'pirate_frb dev coverage'
// reports the marginals: a kernel test that picks a random key can only ever reach the
// (dtype, rank, Wmax, ...) combinations stocked here, and a combination that is absent is one
// no amount of test iterations will cover.
constexpr const char *registry_keys_doc =
    "Every key in this build's kernel registry, as a list of dicts.\n\n"
    "Each dict holds the key's fields verbatim. The registry is populated at load time by\n"
    "the autogenerated kernels, so this reports what the build compiled -- which is what\n"
    "bounds the reach of any test that draws a random key. Used by 'pirate_frb dev coverage'.";
...
void register_kernel_bindings(pybind11::module &m)
{
    py::class_<CoalescedDdKernel2>(m, "CoalescedDdKernel2")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: CoalescedDdKernel2::registry().get_all_keys()) {
                  py::dict d;
                  d["dtype"] = k.dtype;
                  d["dd_rank"] = k.dd_rank;
                  d["Tinner"] = k.Tinner;
                  d["Wmax"] = k.Wmax;
                  d["subband_counts"] = k.subband_counts;
                  ret.append(d);
              }
              return ret;
          }, registry_keys_doc)
    ...
    py::class_<GpuSbDedispersionKernel>(m, "GpuSbDedispersionKernel",
        ...
        "to autogenerated_sbdd_kernels() in makefile_helper.py.")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: GpuSbDedispersionKernel::registry().get_all_keys()) {
                  py::dict d;
                  d["dd_rank"] = k.dd_rank;
                  d["subband_counts"] = k.subband_counts;
                  ret.append(d);
    ...
    py::class_<GpuDedispersionKernel>(m, "GpuDedispersionKernel",
        ...
        "Unlike GpuSbDedispersionKernel, this kernel has no 'sb_out' (subband) output.")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: GpuDedispersionKernel::registry().get_all_keys()) {
                  py::dict d;
                  d["dtype"] = k.dtype;
                  d["rank"] = k.rank;
                  d["nspec"] = k.nspec;
                  d["input_is_ringbuf"] = k.input_is_ringbuf;
                  d["output_is_ringbuf"] = k.output_is_ringbuf;
                  d["apply_input_residual_lags"] = k.apply_input_residual_lags;
                  ret.append(d);
              }
              return ret;
          }, registry_keys_doc)
    ...
    py::class_<GpuPeakFindingKernel>(m, "GpuPeakFindingKernel")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: GpuPeakFindingKernel::registry().get_all_keys()) {
                  py::dict d;
                  d["dtype"] = k.dtype;
                  d["subband_counts"] = k.subband_counts;
                  d["K"] = k.K;
    ...
    py::class_<PfOutputMicrokernel>(m, "PfOutputMicrokernel")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: PfOutputMicrokernel::registry().get_all_keys()) {
                  py::dict d;
                  d["dtype"] = k.dtype;
                  d["Dout"] = k.Dout;
                  ret.append(d);
    ...
    py::class_<PfWeightReaderMicrokernel>(m, "PfWeightReaderMicrokernel")
          ...
          .def_static("registry_keys", []() {
              py::list ret;
              for (const auto &k: PfWeightReaderMicrokernel::registry().get_all_keys()) {
                  py::dict d;
                  d["dtype"] = k.dtype;
                  d["subband_counts"] = k.subband_counts;
                  d["K"] = k.K;
